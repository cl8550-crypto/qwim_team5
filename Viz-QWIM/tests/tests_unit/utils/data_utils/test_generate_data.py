...
class Test_Generate_Data_Main_Function:
    ...
    @pytest.mark.unit()
    def test_resolve_project_root_boolean_fallback_depth_uses_default(self, tmp_path, monkeypatch):
        """Boolean fallback_depth should use the established three-parent default."""
        ...
        assert module._resolve_project_root(fallback_depth = True) == module._resolve_project_root(fallback_depth = 3)
...
class Test_Generate_Scenarios_Daily_Returns_CMA_Tier_0:
    ...
    @pytest.mark.unit()
    def test_boolean_random_seed_uses_default_seed_path(
        self,
        generate_scenarios_daily_returns_CMA_Tier_0,
    ):
        """Boolean random seeds should fall back to the established default seed."""
        result_bool_seed = generate_scenarios_daily_returns_CMA_Tier_0(
            start_date="2024-01-01",
            end_date="2024-06-30",
            num_scenarios=10,
            random_seed=True,
        )
        result_default_seed = generate_scenarios_daily_returns_CMA_Tier_0(
            start_date="2024-01-01",
            end_date="2024-06-30",
            num_scenarios=10,
            random_seed=42,
        ...
        values_bool_seed = result_bool_seed["Scenarios"]["Equities"].to_list()
        values_default_seed = result_default_seed["Scenarios"]["Equities"].to_list()

        assert np.allclose(values_bool_seed, values_default_seed, rtol=1e-10)
    ...
    @pytest.mark.unit()
    def test_cholesky_fallback_clips_eigenvalues_once(self, monkeypatch):
        """Test that a Cholesky failure falls back to eigenvalue clipping."""
        import src.utils.data_utils.generate_data as module

        original_cholesky = module.np.linalg.cholesky
        count_calls_cholesky = {"value": 0}
        messages_warning = []

        def fake_cholesky(matrix_input):
            count_calls_cholesky["value"] += 1
            if count_calls_cholesky["value"] == 1:
                raise np.linalg.LinAlgError("forced first cholesky failure")
            return original_cholesky(matrix_input)

        def fake_warning(message_text):
            messages_warning.append(message_text)

        monkeypatch.setattr(module.np.linalg, "cholesky", fake_cholesky)
        monkeypatch.setattr(module._logger, "warning", fake_warning)
        ...
        assert count_calls_cholesky["value"] >= 2
        assert messages_warning == ["Covariance matrix was not PSD; eigenvalues clipped to 1e-10"]
        assert result["Scenarios"].shape[0] > 0

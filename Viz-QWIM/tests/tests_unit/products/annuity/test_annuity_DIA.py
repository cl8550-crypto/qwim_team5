...
class Test_Annuity_DIA_Validation:
    ...
    @pytest.mark.unit()
    def test_bool_guarantee_period_years_rejected(self) -> None:
        """Boolean guarantee_period_years should not be accepted as an integer."""
        with pytest.raises(Exception_Validation_Input):
            Annuity_DIA(
                client_age=55,
                annuity_payout_rate=0.08,
                age_income_start=65,
                guarantee_period_years=True,
            )

    @pytest.mark.unit()
    def test_bool_cola_rate_rejected(self) -> None:
        """Boolean COLA rates should not be accepted as numeric rates."""
        with pytest.raises(Exception_Validation_Input):
            Annuity_DIA(
                client_age=55,
                annuity_payout_rate=0.08,
    ...
    @pytest.mark.unit()
    def test_bool_mortality_credit_rejected(self) -> None:
        """Boolean mortality-credit rates should not be accepted as numeric rates."""
        with pytest.raises(Exception_Validation_Input):
            Annuity_DIA(
                client_age=55,
                annuity_payout_rate=0.08,
                age_income_start=65,
                rate_mortality_credit=True,
            )

    @pytest.mark.unit()
    def test_bool_payment_frequency_rejected(self) -> None:
        """Boolean payment_frequency should not be accepted as a positive integer."""
        with pytest.raises(Exception_Validation_Input):
            Annuity_DIA(
                client_age=55,
                annuity_payout_rate=0.08,
...
class Test_Annuity_DIA_Payout_Calculations:
    ...
    def test_calc_annuity_payout_during_deferral(
        ...
        payout = dia_defaults.calc_annuity_payout(amount_principal = 100_000.0)
    ...
    def test_calc_annuity_payout_invalid_principal(
        ...
        with pytest.raises(Exception_Validation_Input):
            dia_defaults.calc_annuity_payout(amount_principal = 0.0)

    @pytest.mark.unit()
    def test_calc_payout_in_year_basic(self, dia_defaults: Annuity_DIA) -> None:
        """Year 1 payout = principal * payout rate (no COLA)."""
        payout = dia_defaults.calc_payout_in_year(amount_principal = 100_000.0, year_number = 1)
    ...
    def test_calc_payout_in_year_with_cola(
        ...
        payout = dia_with_cola.calc_payout_in_year(amount_principal = 100_000.0, year_number = 5)
    ...
    def test_calc_payout_in_year_invalid_year(
        ...
        """Year < 1 should raise."""
        with pytest.raises(Exception_Validation_Input):
            dia_defaults.calc_payout_in_year(amount_principal = 100_000.0, year_number = 0)
    ...
    def test_calc_monthly_payout_during_deferral(
        ...
        monthly = dia_defaults.calc_monthly_payout(amount_principal = 100_000.0)
...
class Test_Annuity_DIA_Analytical_Calculations:
    ...
    def test_calc_future_value(self, dia_defaults: Annuity_DIA) -> None:
        ...
        fv = dia_defaults.calc_future_value(amount_principal = 100_000.0, growth_rate = 0.05)
    ...
    def test_calc_future_value_zero_growth(
        ...
        fv = dia_defaults.calc_future_value(amount_principal = 100_000.0, growth_rate = 0.0)
    ...
    def test_calc_future_value_invalid_principal(
        ...
        """Non-positive principal should raise."""
        with pytest.raises(Exception_Validation_Input):
            dia_defaults.calc_future_value(amount_principal = 0.0, growth_rate = 0.05)
    ...
    def test_calc_future_value_invalid_growth_rate(
        ...
        with pytest.raises(Exception_Validation_Input):
            dia_defaults.calc_future_value(amount_principal = 100_000.0, growth_rate = "five")  # type: ignore[arg-type]
    ...
    def test_calc_present_value_deferred_income_invalid_inputs(
        ...
        with pytest.raises(Exception_Validation_Input):
            dia_defaults.calc_present_value_deferred_income(amount_principal = 0.0, discount_rate = 0.04, life_expectancy_years = 30)
        with pytest.raises(Exception_Validation_Input):
            dia_defaults.calc_present_value_deferred_income(amount_principal = 100_000.0, discount_rate = -0.01, life_expectancy_years = 30)
        with pytest.raises(Exception_Validation_Input):
            dia_defaults.calc_present_value_deferred_income(amount_principal = 100_000.0, discount_rate = 0.04, life_expectancy_years = 0)
    ...
    def test_calc_commutation_value_invalid_inputs(
        ...
        """Invalid inputs should raise."""
        with pytest.raises(Exception_Validation_Input):
            dia_defaults.calc_commutation_value(amount_principal = 0.0, discount_rate = 0.04, remaining_payment_years = 15)
        with pytest.raises(Exception_Validation_Input):
            dia_defaults.calc_commutation_value(amount_principal = 100_000.0, discount_rate = -0.01, remaining_payment_years = 15)
        with pytest.raises(Exception_Validation_Input):
            dia_defaults.calc_commutation_value(amount_principal = 100_000.0, discount_rate = 0.04, remaining_payment_years = 0)
    ...
    def test_calc_breakeven_age(self, dia_defaults: Annuity_DIA) -> None:
        ...
        breakeven_age = dia_defaults.calc_breakeven_age(amount_principal = 100_000.0)
    ...
    def test_calc_breakeven_age_invalid_principal(
        ...
        """Non-positive principal should raise."""
        with pytest.raises(Exception_Validation_Input):
            dia_defaults.calc_breakeven_age(amount_principal = 0.0)
    ...
    def test_calc_breakeven_not_reached(self) -> None:
        ...
        with pytest.raises(Exception_Calculation):
            dia.calc_breakeven_age(amount_principal = 100_000.0)
    ...
    def test_calc_mortality_credit_value(
        ...
        mc = dia_with_rop.calc_mortality_credit_value(amount_principal = 100_000.0)
    ...
    def test_calc_mortality_credit_zero_rate(
        ...
        mc = dia_defaults.calc_mortality_credit_value(amount_principal = 100_000.0)
    ...
    def test_calc_death_benefit_with_rop(
        ...
        db = dia_with_rop.calc_death_benefit_during_deferral(amount_principal = 100_000.0)
    ...
    def test_calc_death_benefit_without_rop(
        ...
        db = dia_defaults.calc_death_benefit_during_deferral(amount_principal = 100_000.0)
    ...
    def test_calc_death_benefit_invalid_principal(
        ...
        """Non-positive principal should raise."""
        with pytest.raises(Exception_Validation_Input):
            dia_with_rop.calc_death_benefit_during_deferral(amount_principal = 0.0)
...
class Test_Annuity_DIA_Withdrawal_Rates:
    ...
    def test_returns_withdrawal_rates_struct(
        ...
        result = dia_defaults.calc_withdrawal_rates(amount_principal = 100_000.0, desired_WR=0.04)
    ...
    def test_nominal_wr_positive_past_income_age(self) -> None:
        ...
        result = dia.calc_withdrawal_rates(amount_principal = 100_000.0, desired_WR=0.04)
    ...
    def test_invalid_principal_zero_raises(
        ...
        with pytest.raises(Exception_Validation_Input):
            dia_defaults.calc_withdrawal_rates(amount_principal = 0.0, desired_WR=0.04)
    ...
    def test_invalid_principal_negative_raises(
        ...
        with pytest.raises(Exception_Validation_Input):
            dia_defaults.calc_withdrawal_rates(amount_principal = -1.0, desired_WR=0.04)
    ...
    def test_invalid_desired_wr_zero_raises(
        ...
        with pytest.raises(Exception_Validation_Input):
            dia_defaults.calc_withdrawal_rates(amount_principal = 100_000.0, desired_WR=0.0)
    ...
    def test_invalid_desired_wr_negative_raises(
        ...
        with pytest.raises(Exception_Validation_Input):
            dia_defaults.calc_withdrawal_rates(amount_principal = 100_000.0, desired_WR=-0.04)
...
@pytest.mark.unit()
class Test_Annuity_DIA_Coverage_Gaps:
    """Target the remaining DIA validation and branch gaps."""

    @pytest.mark.unit()
    def test_negative_guarantee_period_years_raises(self) -> None:
        """Negative guarantee_period_years should raise Exception_Validation_Input."""
        with pytest.raises(Exception_Validation_Input):
            Annuity_DIA(
                client_age=55,
                annuity_payout_rate=0.08,
    ...
    @pytest.mark.unit()
    def test_calc_annuity_payout_applies_inflation_after_deferral(
        self,
        dia_inflation_adjusted: Annuity_DIA,
        inflation_df_single: pl.DataFrame,
    ) -> None:
        """Inflation-adjusted DIA should apply the cumulative factor once income starts."""
        dia_inflation_adjusted.m_client_age = dia_inflation_adjusted.m_age_income_start
        ...
        assert payout == pytest.approx(100_000.0 * 0.08 * 1.03)

    @pytest.mark.unit()
    def test_calc_annuity_payout_invalid_inflation_schema_raises_after_deferral(
        self,
        dia_inflation_adjusted: Annuity_DIA,
    ) -> None:
        """Bad inflation columns should be rejected once the DIA is past deferral."""
        dia_inflation_adjusted.m_client_age = dia_inflation_adjusted.m_age_income_start
        bad_df = pl.DataFrame(
            {
                "Start Date": ["2024-01-01"],
        ...
        with pytest.raises(Exception_Validation_Input):
            dia_inflation_adjusted.calc_annuity_payout(
                amount_principal = 100_000.0,
                obj_inflation=bad_df,
            )

    @pytest.mark.unit()
    def test_calc_payout_in_year_invalid_principal_raises(
        self,
        dia_defaults: Annuity_DIA,
    ) -> None:
        """calc_payout_in_year should reject non-positive principal."""
        with pytest.raises(Exception_Validation_Input):
            dia_defaults.calc_payout_in_year(amount_principal = 0.0, year_number = 1)

    @pytest.mark.unit()
    def test_present_value_period_certain_uses_guarantee_years(self) -> None:
        """Period-certain DIA should value exactly the guaranteed payment horizon."""
        dia = Annuity_DIA(
            client_age=55,
            annuity_payout_rate=0.08,
            age_income_start=65,
        ...
        pv = dia.calc_present_value_deferred_income(
            amount_principal=100_000.0,
            discount_rate=0.04,
            life_expectancy_years=30,
        )

        expected = sum(8_000.0 / (1.04**t) for t in range(11, 21))
        assert pv == pytest.approx(expected, rel=1e-9)

    @pytest.mark.unit()
    def test_present_value_life_with_period_certain_uses_max_payment_horizon(self) -> None:
        """Life-with-period-certain should use the larger of guarantee and remaining life."""
        life_only = Annuity_DIA(
            client_age=55,
            annuity_payout_rate=0.08,
            age_income_start=65,
            payout_option=Annuity_Payout_Option.LIFE_ONLY,
        )
        life_with_guarantee = Annuity_DIA(
            client_age=55,
            annuity_payout_rate=0.08,
            age_income_start=65,
            payout_option=Annuity_Payout_Option.LIFE_WITH_PERIOD_CERTAIN,
        ...
        life_only_pv = life_only.calc_present_value_deferred_income(
            amount_principal=100_000.0,
            discount_rate=0.04,
            life_expectancy_years=15,
        )
        guaranteed_pv = life_with_guarantee.calc_present_value_deferred_income(
            amount_principal=100_000.0,
            discount_rate=0.04,
            life_expectancy_years=15,
        )

        assert guaranteed_pv > life_only_pv

    @pytest.mark.unit()
    def test_calc_mortality_credit_value_invalid_principal_raises(
        self,
        dia_defaults: Annuity_DIA,
    ) -> None:
        """calc_mortality_credit_value should reject non-positive principal."""
        with pytest.raises(Exception_Validation_Input):
            dia_defaults.calc_mortality_credit_value(amount_principal = 0.0)

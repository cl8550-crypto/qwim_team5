...
class Test_Annuity_VA_Validation:
    ...
    @pytest.mark.parametrize(
        ("invalid_kwargs",),
        [
            pytest.param({"rate_rollup_benefit": True}, id="bool_rate_rollup_benefit"),
            pytest.param({"rate_ME_charge": True}, id="bool_rate_ME_charge"),
            pytest.param({"rate_admin_fee": True}, id="bool_rate_admin_fee"),
            ...
            pytest.param({"pct_GLWB": True}, id="bool_pct_GLWB"),
    ...
    @pytest.mark.unit()
    def test_negative_surrender_charge_schedule_years(self) -> None:
        """Negative surrender schedule years should raise."""
        with pytest.raises(Exception_Validation_Input):
            Annuity_VA(
                client_age=55,
                annuity_payout_rate=0.05,
...
class Test_Annuity_VA_Charge_Calculations:
    ...
    def test_calc_account_value_after_charges_single_period(
        ...
        values = va_defaults.calc_account_value_after_charges(amount_principal = 100_000.0, gross_returns = [0.10])
        assert len(values) == 2
    ...
    def test_calc_account_value_invalid_principal(
        ...
        with pytest.raises(Exception_Validation_Input):
            va_defaults.calc_account_value_after_charges(amount_principal = 0.0, gross_returns = [0.05])
    ...
    def test_calc_account_value_invalid_returns_type(
        ...
        with pytest.raises(Exception_Validation_Input):
            va_defaults.calc_account_value_after_charges(amount_principal = 100_000.0, gross_returns = 0.05)  # type: ignore[arg-type]
...
class Test_Annuity_VA_Surrender:
    ...
    def test_surrender_charge_year_zero(self, va_defaults: Annuity_VA) -> None:
        ...
        rate = va_defaults.calc_surrender_charge_rate(year = 0)
    ...
    def test_surrender_charge_declining(self, va_defaults: Annuity_VA) -> None:
        ...
        rate = va_defaults.calc_surrender_charge_rate(year = 3)
        expected = 0.07 * (1 - 3 / 7)
    ...
    def test_surrender_charge_after_schedule(
        ...
        assert va_defaults.calc_surrender_charge_rate(year = 7) == pytest.approx(0.0)
        assert va_defaults.calc_surrender_charge_rate(year = 20) == pytest.approx(0.0)
    ...
    def test_surrender_charge_negative_year(
        ...
        assert va_defaults.calc_surrender_charge_rate(year = -1) == pytest.approx(0.0)
    ...
    def test_surrender_charge_zero_schedule_returns_zero(
        ...
        """A zero-year surrender schedule disables surrender charges."""
        va_defaults.m_surrender_charge_schedule_years = 0
        assert va_defaults.calc_surrender_charge_rate(year = 0) == pytest.approx(0.0)

    @pytest.mark.unit()
    def test_surrender_value_year_zero(self, va_defaults: Annuity_VA) -> None:
        """Surrender value with full charge."""
        sv = va_defaults.calc_surrender_value(account_value = 100_000.0, year = 0)
    ...
    def test_surrender_value_after_schedule(
        ...
        sv = va_defaults.calc_surrender_value(account_value = 100_000.0, year = 10)
    ...
    def test_surrender_value_invalid_account_value(
        ...
        with pytest.raises(Exception_Validation_Input):
            va_defaults.calc_surrender_value(account_value = 0.0, year = 3)
...
class Test_Annuity_VA_Benefit_Base:
    ...
    def test_calc_benefit_base_default_deferral(
        ...
        bb = va_defaults.calc_benefit_base_with_rollup(amount_principal = 100_000.0)
    ...
    def test_calc_benefit_base_custom_years(
        ...
        bb = va_defaults.calc_benefit_base_with_rollup(amount_principal = 100_000.0, years_deferred=5)
    ...
    def test_calc_benefit_base_zero_years(
        ...
        bb = va_defaults.calc_benefit_base_with_rollup(amount_principal = 100_000.0, years_deferred=0)
    ...
    def test_calc_benefit_base_invalid_principal(
        ...
        """Non-positive principal should raise."""
        with pytest.raises(Exception_Validation_Input):
            va_defaults.calc_benefit_base_with_rollup(amount_principal = 0.0)

    @pytest.mark.unit()
    def test_calc_benefit_base_invalid_years_deferred(
        self,
        va_defaults: Annuity_VA,
    ) -> None:
        """Negative deferral years should raise."""
        with pytest.raises(Exception_Validation_Input):
            va_defaults.calc_benefit_base_with_rollup(amount_principal = 100_000.0, years_deferred=-1)
    ...
    def test_calc_death_benefit_with_gmdb(
        ...
        db = va_defaults.calc_death_benefit(amount_principal = 100_000.0, account_value = 80_000.0)
        assert db == pytest.approx(100_000.0)

        # Account gained value → use account value
        db = va_defaults.calc_death_benefit(amount_principal = 100_000.0, account_value = 120_000.0)
    ...
    def test_calc_death_benefit_without_gmdb(
        ...
        db = va_no_riders.calc_death_benefit(amount_principal = 100_000.0, account_value = 80_000.0)
    ...
    def test_calc_death_benefit_invalid_principal(
        ...
        """Non-positive principal should raise."""
        with pytest.raises(Exception_Validation_Input):
            va_defaults.calc_death_benefit(amount_principal = 0.0, account_value = 80_000.0)
    ...
    def test_calc_death_benefit_negative_account(
        ...
        with pytest.raises(Exception_Validation_Input):
            va_defaults.calc_death_benefit(amount_principal = 100_000.0, account_value = -1.0)

    @pytest.mark.unit()
    def test_calc_GLWB_payout(self, va_defaults: Annuity_VA) -> None:
        """GLWB payout = benefit_base * pct_GLWB."""
        payout = va_defaults.calc_GLWB_payout(amount_principal = 100_000.0)
    ...
    def test_calc_GLWB_payout_custom_deferral(
        ...
        payout = va_defaults.calc_GLWB_payout(amount_principal = 100_000.0, years_deferred=5)
    ...
    def test_calc_GLWB_payout_without_glwb(
        ...
        payout = va_no_riders.calc_GLWB_payout(amount_principal = 100_000.0)
    ...
    def test_calc_GLWB_payout_invalid_principal(
        ...
        """Non-positive principal should raise."""
        with pytest.raises(Exception_Validation_Input):
            va_defaults.calc_GLWB_payout(amount_principal = 0.0)

    @pytest.mark.unit()
    def test_calc_GLWB_payout_negative_result_raises(
        self,
        va_defaults: Annuity_VA,
    ) -> None:
        """A negative computed payout should raise a calculation error."""
        va_defaults.m_pct_GLWB = -0.05
        with pytest.raises(Exception_Calculation):
            va_defaults.calc_GLWB_payout(amount_principal = 100_000.0)
...
class Test_Annuity_VA_Payout_Calculations:
    ...
    def test_calc_annuity_payout_during_deferral(
        ...
        payout = va_defaults.calc_annuity_payout(amount_principal = 100_000.0)
        assert payout == pytest.approx(0.0)
    ...
    def test_calc_annuity_payout_invalid_principal(
        ...
        """Non-positive principal should raise."""
        with pytest.raises(Exception_Validation_Input):
            va_defaults.calc_annuity_payout(amount_principal = 0.0)
    ...
    def test_calc_monthly_payout_during_deferral(
        ...
        monthly = va_defaults.calc_monthly_payout(amount_principal = 100_000.0)
...
class Test_Annuity_VA_Inflation_Parameter:
    ...
    def test_calc_annuity_payout_accepts_none(
        ...
        payout = va_defaults.calc_annuity_payout(amount_principal = 100_000.0, obj_inflation=None)
        assert payout == pytest.approx(0.0)
...
class Test_Annuity_VA_Scenarios_Parameter:
    ...
    @pytest.mark.unit()
    def test_calc_annuity_payout_with_scenarios_uses_ratchet_max(
        self,
        scenarios_df_va: pl.DataFrame,
    ) -> None:
        """Before the ratchet window closes, payout uses max(rollup, market)."""
        va = Annuity_VA(
            client_age=64,
            annuity_payout_rate=0.05,
            age_income_start=65,
        ...
        payout = va.calc_annuity_payout(amount_principal = 100_000.0, obj_scenarios=scenarios_df_va)
        account_values = va.calc_account_value_after_charges(amount_principal = 100_000.0, gross_returns = [0.08, 0.10])
        expected = max(100_000.0, account_values[-1]) * 0.05
        assert payout == pytest.approx(expected, rel=1e-9)

    @pytest.mark.unit()
    def test_calc_annuity_payout_past_income_age_without_scenarios(
        self,
    ) -> None:
        """Without scenarios, payout falls back to the rollup benefit base."""
        va = Annuity_VA(
            client_age=64,
            annuity_payout_rate=0.05,
            age_income_start=65,
        ...
        payout = va.calc_annuity_payout(amount_principal = 100_000.0)
        expected = 100_000.0 * 0.05
        assert payout == pytest.approx(expected, rel=1e-9)
    ...
    def test_calc_annuity_payout_missing_date_column(
        ...
        with pytest.raises(Exception_Validation_Input):
            va_past_income_age.calc_annuity_payout(amount_principal = 100_000.0, obj_scenarios=bad_df)
    ...
    def test_calc_annuity_payout_custom_proxy_column(
        ...
        with pytest.raises(Exception_Validation_Input):
            va_past_income_age.calc_annuity_payout(amount_principal = 100_000.0, obj_scenarios=wrong_df)
...
class Test_Annuity_VA_Withdrawal_Rates:
    ...
    def test_returns_withdrawal_rates_struct(
        ...
        result = va_defaults.calc_withdrawal_rates(amount_principal = 100_000.0, desired_WR=0.04)
    ...
    def test_invalid_principal_zero_raises(
        ...
        with pytest.raises(Exception_Validation_Input):
            va_defaults.calc_withdrawal_rates(amount_principal = 0.0, desired_WR=0.04)
    ...
    def test_invalid_principal_negative_raises(
        ...
        with pytest.raises(Exception_Validation_Input):
            va_defaults.calc_withdrawal_rates(amount_principal = -1.0, desired_WR=0.04)
    ...
    def test_invalid_desired_wr_zero_raises(
        ...
        with pytest.raises(Exception_Validation_Input):
            va_defaults.calc_withdrawal_rates(amount_principal = 100_000.0, desired_WR=0.0)
    ...
    def test_invalid_desired_wr_negative_raises(
        ...
        with pytest.raises(Exception_Validation_Input):
            va_defaults.calc_withdrawal_rates(amount_principal = 100_000.0, desired_WR=-0.04)

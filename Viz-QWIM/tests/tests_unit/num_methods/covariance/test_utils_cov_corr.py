...
class Test_Covariance_Matrix_Accessors:
    ...
    def test_get_component_variance_returns_float(self, cov_matrix_instance):
        ...
        result = cov_matrix_instance.get_component_variance(component_name = "AAPL")
    ...
    def test_get_component_variance_positive(self, cov_matrix_instance):
        ...
        for name in cov_matrix_instance.m_component_names:
            variance = cov_matrix_instance.get_component_variance(component_name = name)
    ...
    def test_get_component_variance_unknown_name(
        ...
        with pytest.raises(Exception_Validation_Input_class, match="not found"):
            cov_matrix_instance.get_component_variance(component_name = "UNKNOWN_TICKER")
    ...
    def test_get_component_covariance_returns_float(self, cov_matrix_instance):
        ...
        result = cov_matrix_instance.get_component_covariance(component_name_1 = "AAPL", component_name_2 = "MSFT")
        assert isinstance(result, float)

    @pytest.mark.unit()
    def test_get_component_covariance_symmetric(self, cov_matrix_instance):
        """Test that covariance(A, B) == covariance(B, A)."""
        cov_ab = cov_matrix_instance.get_component_covariance(component_name_1 = "AAPL", component_name_2 = "MSFT")
        cov_ba = cov_matrix_instance.get_component_covariance(component_name_1 = "MSFT", component_name_2 = "AAPL")
    ...
    def test_get_component_covariance_self_equals_variance(self, cov_matrix_instance):
        ...
        for name in cov_matrix_instance.m_component_names:
            cov_self = cov_matrix_instance.get_component_covariance(component_name_1 = name, component_name_2 = name)
            variance = cov_matrix_instance.get_component_variance(component_name = name)
    ...
    def test_get_component_covariance_unknown_first(
        ...
        with pytest.raises(Exception_Validation_Input_class, match="not found"):
            cov_matrix_instance.get_component_covariance(component_name_1 = "UNKNOWN", component_name_2 = "AAPL")
    ...
    def test_get_component_covariance_unknown_second(
        ...
        with pytest.raises(Exception_Validation_Input_class, match="not found"):
            cov_matrix_instance.get_component_covariance(component_name_1 = "AAPL", component_name_2 = "UNKNOWN")
...
# ---------------------------------------------------------------------------
# Phase-3 gap-filling tests  (lines 391-392, 434-437, 448-451, 494-496)
# ---------------------------------------------------------------------------


def _make_returns_df(num_obs: int, num_assets: int, seed: int = 99) -> pl.DataFrame:
    """Helper: build a minimal valid returns DataFrame."""
    import numpy as np
    ...
    asset_names = [f"A{i}" for i in range(num_assets)]
    data: dict = {"Date": list(range(num_obs))}
    for idx, name in enumerate(asset_names):
        data[name] = returns[:, idx].tolist()

    # Use plain Python date objects via polars
    import datetime as dt

    start = dt.date(2023, 1, 1)
    dates = [start + dt.timedelta(days=i) for i in range(num_obs)]
    data["Date"] = dates
    return pl.DataFrame(data)


class Test_Insufficient_Observations_Warning:
    """Cover lines 391-392 -- warning when obs < num_components (but obs >= 3)."""

    @pytest.mark.unit()
    def test_obs_less_than_components_emits_warning(
        self, covariance_matrix_class, covariance_estimator_enum,
    ):
        """3 obs for 4 components triggers the warning path (lines 391-392)."""
        df = _make_returns_df(num_obs=3, num_assets=4)
        # Instantiation should succeed (only a warning, no raise)
        with pytest.warns(UserWarning):
            obj = covariance_matrix_class(
                data_returns=df,
                estimator=covariance_estimator_enum.EMPIRICAL,
            )
        assert obj.m_num_observations == 3
        assert obj.m_num_components == 4


class Test_Estimation_Exception_Handler:
    """Cover lines 434-437 and 494-496."""

    @pytest.mark.unit()
    def test_unknown_estimator_raises_directly(
        self, covariance_matrix_class, covariance_estimator_enum, sample_returns_df,
    ):
        """Calling _estimate_covariance with a non-enum estimator covers lines 494-496."""
        ...
        # Build a valid instance first, then call the private method directly
        obj = covariance_matrix_class(
            data_returns=sample_returns_df,
            estimator=covariance_estimator_enum.EMPIRICAL,
        )
        returns_np = sample_returns_df.select(
            [c for c in sample_returns_df.columns if c != "Date"],
        ).to_numpy()
        # MagicMock has .value attribute so error_msg formatting succeeds
        fake_estimator = MagicMock(value="UNKNOWN_ESTIMATOR")
        with pytest.raises(Exception_Validation_Input, match="Unknown estimator type"):
            obj._estimate_covariance(returns = returns_np, estimator = fake_estimator)  # not in map

    @pytest.mark.unit()
    def test_estimation_failure_caught_and_rewrapped(
        self, covariance_matrix_class, covariance_estimator_enum,
    ):
        """Patching _estimate_covariance to raise covers lines 434-437 (outer except)."""
        ...
        df = _make_returns_df(num_obs=50, num_assets=2)
        with (
            patch.object(
                covariance_matrix_class,
            ...
            pytest.raises(Exception_Calculation, match="Covariance estimation failed"),
        ):
                covariance_matrix_class(
                    data_returns=df,
                    estimator=covariance_estimator_enum.EMPIRICAL,
...
class Test_Validation_Exception_Handler:
    """Cover lines 448-451 -- exception when validate_covariance_matrix raises."""

    @pytest.mark.unit()
    def test_validation_failure_caught_and_rewrapped(
        self, covariance_matrix_class, covariance_estimator_enum,
    ):
        """Patching _estimate_covariance to return NaN triggers validation → lines 448-451."""
        from unittest.mock import patch
        ...
        with (
            patch.object(
                covariance_matrix_class,
                "_estimate_covariance",
                return_value=nan_matrix,
            ),
            pytest.raises(Exception_Calculation, match="failed sanity checks"),
        ):
                covariance_matrix_class(
                    data_returns=df,
                    estimator=covariance_estimator_enum.EMPIRICAL,
...
# ===========================================================================
# covariance_matrix - to_polars method tests
# ===========================================================================


class Test_Covariance_Matrix_To_Polars:
    """Tests for the ``to_polars()`` method.

    These tests verify that the method returns a correctly shaped and labelled
    Polars DataFrame.  Without them, lines 892-897 of ``utils_cov_corr.py``
    (the entire ``to_polars`` body) are never executed.
    """

    @pytest.mark.unit()
    def test_returns_polars_dataframe(self, cov_matrix_instance):
        """to_polars() must return a polars.DataFrame."""
        result = cov_matrix_instance.to_polars()
        assert isinstance(result, pl.DataFrame)

    @pytest.mark.unit()
    def test_shape_is_n_by_n_plus_one(self, cov_matrix_instance):
        """to_polars() shape must be (n_components, n_components + 1)."""
        n = cov_matrix_instance.m_num_components
        result = cov_matrix_instance.to_polars()
        assert result.shape == (n, n + 1)

    @pytest.mark.unit()
    def test_first_column_is_component(self, cov_matrix_instance):
        """First column of to_polars() result must be named 'component'."""
        result = cov_matrix_instance.to_polars()
        assert result.columns[0] == "component"

    @pytest.mark.unit()
    def test_component_column_contains_names(self, cov_matrix_instance):
        """The 'component' column must contain the component names."""
        result = cov_matrix_instance.to_polars()
        assert result["component"].to_list() == cov_matrix_instance.m_component_names

    @pytest.mark.unit()
    def test_value_columns_match_component_names(self, cov_matrix_instance):
        """Non-component columns must be named after each component."""
        result = cov_matrix_instance.to_polars()
        value_cols = result.columns[1:]
        assert value_cols == cov_matrix_instance.m_component_names

    @pytest.mark.unit()
    def test_diagonal_matches_variance(self, cov_matrix_instance):
        """Diagonal of the output matrix must match the covariance diagonal."""
        result = cov_matrix_instance.to_polars()
        for idx_col, name in enumerate(cov_matrix_instance.m_component_names):
            actual_diag = result[idx_col, name]
            expected_diag = float(cov_matrix_instance.m_cov_matrix[idx_col, idx_col])
            assert actual_diag == pytest.approx(expected_diag, rel=1e-10)

    @pytest.mark.unit()
    def test_values_match_cov_matrix(self, cov_matrix_instance):
        """All values in to_polars() must match the underlying NumPy matrix."""
        result = cov_matrix_instance.to_polars()
        for idx_row, _item_row_name in enumerate(cov_matrix_instance.m_component_names):
            for idx_col, col_name in enumerate(cov_matrix_instance.m_component_names):
                actual = result[idx_row, col_name]
                expected = float(cov_matrix_instance.m_cov_matrix[idx_row, idx_col])

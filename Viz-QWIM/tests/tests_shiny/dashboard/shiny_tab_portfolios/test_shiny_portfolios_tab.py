...
try:
    ...
    _logger.warning("pytest-playwright not installed — Shiny UI tests will be skipped")
...
class Test_Portfolio_Analysis_Subtab_UI:
    ...
    def test_data_info_output_rendered(
        self, page: Page, shiny_server_url: str,
    ) -> None:
        """Data info output container is rendered in the Analysis subtab."""
        url = _base_url(shiny_server_url)
        _navigate_to_portfolios_tab(page, url)
        _click_subtab(page, _SEL_SUBTAB_ANALYSIS)
        data_info = page.locator(f"#{_ID_ANALYSIS_DATA_INFO}, [id='{_ID_ANALYSIS_DATA_INFO}']")
        assert data_info.count() > 0, f"Data info output '{_ID_ANALYSIS_DATA_INFO}' not found"
        _logger.info("Analysis data info output found")

    def test_statistics_tables_rendered(
        self, page: Page, shiny_server_url: str,
    ) -> None:
        """Statistics and metrics table outputs are rendered in the Analysis subtab."""
        url = _base_url(shiny_server_url)
        _navigate_to_portfolios_tab(page, url)
        _click_subtab(page, _SEL_SUBTAB_ANALYSIS)
        loading_status = page.locator(
            f"#{_ID_ANALYSIS_LOADING_STATUS}, [id='{_ID_ANALYSIS_LOADING_STATUS}']",
        )
        stats_table = page.locator(
            f"#{_ID_ANALYSIS_TABLE_STATS}, [id='{_ID_ANALYSIS_TABLE_STATS}']",
        )
        metrics_table = page.locator(
            f"#{_ID_ANALYSIS_TABLE_METRICS}, [id='{_ID_ANALYSIS_TABLE_METRICS}']",
        )
        assert loading_status.count() > 0, (
            f"Loading status output '{_ID_ANALYSIS_LOADING_STATUS}' not found"
        )
        assert stats_table.count() > 0, (
            f"Statistics table output '{_ID_ANALYSIS_TABLE_STATS}' not found"
        )
        assert metrics_table.count() > 0, (
            f"Metrics table output '{_ID_ANALYSIS_TABLE_METRICS}' not found"
        )
        _logger.info("Analysis loading status and table outputs found")
...
@pytest.mark.playwright()
class Class_Test_Weights_Analysis_Subtab_UI:
    ...
    def Test_Weights_Chart_Type_Selector_Visible(
        ...
        _navigate_to_portfolios_tab(page, url)
        _click_subtab(page, _SEL_SUBTAB_WEIGHTS)
        field = _input_locator(page, _ID_WEIGHTS_VIZ_TYPE)
        assert field.count() > 0, (
            f"Weights visualization selector '{_ID_WEIGHTS_VIZ_TYPE}' not found"
        )
        _logger.info("Weights visualization selector found")

    def Test_Weights_Status_And_Summary_Outputs_Rendered(
        self, page: Page, shiny_server_url: str,
    ) -> None:
        """Weights status, plot, and summary output containers are rendered."""
        url = _base_url(shiny_server_url)
        _navigate_to_portfolios_tab(page, url)
        _click_subtab(page, _SEL_SUBTAB_WEIGHTS)
        ...
        for output_id in (
            ...
            locator = page.locator(f"#{output_id}, [id*='{output_id}']")
            assert locator.count() > 0, f"Weights output '{output_id}' not found"

        _logger.info("Weights status, plots, and summary outputs found")

...
class TestValidateDataClientsInReactives:
    ...
    def test_none_returns_false_or_none(self) -> None:
        """Test that none returns false or none."""
        from src.dashboard.shiny_utils.utils_reporting import validate_data_clients_in_reactives

        result = validate_data_clients_in_reactives(reactives_shiny = None)  # type: ignore[arg-type]
    ...
    def test_non_dict_returns_false(self) -> None:
        ...
        ok, _ = validate_data_clients_in_reactives(reactives_shiny = "not-a-dict")  # type: ignore[arg-type]
        assert ok is False
    ...
    def test_missing_data_clients_key_returns_false(self) -> None:
        ...
        ok, msg = validate_data_clients_in_reactives(reactives_shiny = {"Wrong": {}})
        assert ok is False
    ...
    def test_data_clients_not_dict_returns_false(self) -> None:
        ...
        ok, _msg = validate_data_clients_in_reactives(reactives_shiny = {"Data_Clients": "bad"})
        assert ok is False
    ...
    def test_missing_sub_level_returns_false(self, missing_sub_level: str) -> None:
        ...
        ok, _ = validate_data_clients_in_reactives(reactives_shiny = {"Data_Clients": data_clients})
        assert ok is False
    ...
    def test_valid_structure_returns_true(
        ...
        ok, msg = validate_data_clients_in_reactives(reactives_shiny = reactives_with_data_clients)
        assert ok is True
...
class TestValidateClientLevelName:
    ...
    def test_valid_levels(self, valid_level: str) -> None:
        """Test that valid levels."""
        from src.dashboard.shiny_utils.utils_reporting import validate_client_level_name

        ok, msg = validate_client_level_name(client_level = valid_level)
        assert ok is True
        assert isinstance(msg, str)

    def test_none_returns_none_or_false(self) -> None:
        """Test that none returns none or false."""
        from src.dashboard.shiny_utils.utils_reporting import validate_client_level_name

        result = validate_client_level_name(client_level = None)  # type: ignore[arg-type]
        assert result is None or (isinstance(result, tuple) and result[0] is False)

    def test_non_string_returns_none_or_false(self) -> None:
        """Test that non string returns none or false."""
        from src.dashboard.shiny_utils.utils_reporting import validate_client_level_name

        result = validate_client_level_name(client_level = 123)  # type: ignore[arg-type]
        assert result is None or (isinstance(result, tuple) and result[0] is False)

    def test_empty_string_returns_none_or_false(self) -> None:
        """Test that empty string returns none or false."""
        from src.dashboard.shiny_utils.utils_reporting import validate_client_level_name

        result = validate_client_level_name(client_level = "")
        assert result is None or (isinstance(result, tuple) and result[0] is False)
    ...
    def test_unknown_level_returns_false(self) -> None:
        ...
        ok, msg = validate_client_level_name(client_level = "Bad_Level")
        assert ok is False
        assert "Bad_Level" in msg

    def test_case_sensitive(self) -> None:
        """Test that case sensitive."""
        from src.dashboard.shiny_utils.utils_reporting import validate_client_level_name

        ok, _ = validate_client_level_name(client_level = "client_primary")
        assert ok is False
...
class TestValidateClientDataCategoryName:
    ...
    def test_valid_categories_for_primary(self, category: str) -> None:
        """Test that valid categories for primary."""
        from src.dashboard.shiny_utils.utils_reporting import (
            validate_client_data_category_name,
        )

        ok, _ = validate_client_data_category_name(data_category = category, client_level = "Client_Primary")
        assert ok is True

    def test_personal_info_invalid_for_combined(self) -> None:
        """Test that personal info invalid for combined."""
        from src.dashboard.shiny_utils.utils_reporting import (
            validate_client_data_category_name,
        )

        ok, msg = validate_client_data_category_name(data_category = "Personal_Info", client_level = "Clients_Combined")
        assert ok is False
    ...
    def test_shared_categories_valid_for_combined(self, category: str) -> None:
        """Test that shared categories valid for combined."""
        from src.dashboard.shiny_utils.utils_reporting import (
            validate_client_data_category_name,
        )

        ok, _ = validate_client_data_category_name(data_category = category, client_level = "Clients_Combined")
        assert ok is True

    def test_none_category_returns_none_or_false(self) -> None:
        """Test that none category returns none or false."""
        from src.dashboard.shiny_utils.utils_reporting import (
            validate_client_data_category_name,
        )

        result = validate_client_data_category_name(data_category = None)  # type: ignore[arg-type]
        assert result is None or (isinstance(result, tuple) and result[0] is False)
    ...
    def test_unknown_category_returns_false(self) -> None:
        ...
        ok, msg = validate_client_data_category_name(data_category = "Bad_Category")
        assert ok is False
...
class TestSingleOrCouple:
    ...
    def test_default_value_is_single_or_couple(
        ...
        """Test that default value is single or couple."""
        from shiny import reactive
        ...
        with reactive.isolate():
            value = get_single_or_couple_from_reactives(reactives_shiny = reactives_with_data_clients)
    ...
    def test_roundtrip_set_get(
        ...
        with reactive.isolate():
            update_single_or_couple_in_reactives(reactives_shiny = reactives_with_data_clients, single_or_couple = value)
            result = get_single_or_couple_from_reactives(reactives_shiny = reactives_with_data_clients)
        assert result == value

    def test_invalid_value_raises_value_error(
        self, reactives_with_data_clients: dict[str, Any],
    ) -> None:
        """Test that invalid value raises value error."""
        from shiny import reactive
        ...
        with pytest.raises(Exception_Validation_Input, match="Invalid single_or_couple"), reactive.isolate():
            update_single_or_couple_in_reactives(
                reactives_shiny = reactives_with_data_clients, single_or_couple = "Invalid",
            )

    def test_none_value_raises_value_error(
        self, reactives_with_data_clients: dict[str, Any],
    ) -> None:
        """Test that none value raises value error."""
        from shiny import reactive
        ...
        with pytest.raises(Exception_Validation_Input), reactive.isolate():
            update_single_or_couple_in_reactives(
                reactives_shiny = reactives_with_data_clients, single_or_couple = None,  # type: ignore[arg-type]
            )

    def test_invalid_reactives_raises_value_error(self) -> None:
        """Test that invalid reactives raises value error."""
        from shiny import reactive
        ...
        with pytest.raises(Exception_Validation_Input), reactive.isolate():
            get_single_or_couple_from_reactives(reactives_shiny = {"bad": "structure"})
...
class TestGetSaveClientData:
    ...
    def test_save_and_get_none_clears_value(
        ...
        """Test that save and get none clears value."""
        from shiny import reactive
        ...
        with reactive.isolate():
            save_client_data_to_reactives(
                reactives_shiny = reactives_with_data_clients, client_level = client_level, data_category = category, data_value = None,
            )
            result = get_client_data_from_reactives(
                reactives_shiny = reactives_with_data_clients, client_level = client_level, data_category = category,
            )
    ...
    def test_save_and_get_dataframe(
        ...
        """Test that save and get dataframe."""
        import polars as pl
        ...
        with reactive.isolate():
            save_client_data_to_reactives(
                reactives_shiny = reactives_with_data_clients, client_level = "Client_Primary", data_category = "Assets", data_value = sample_assets_df,
            )
            result = get_client_data_from_reactives(
                reactives_shiny = reactives_with_data_clients, client_level = "Client_Primary", data_category = "Assets",
            )
    ...
    def test_invalid_level_raises_value_error(
        ...
        """Test that invalid level raises value error."""
        from shiny import reactive
        ...
        with pytest.raises(Exception_Validation_Input), reactive.isolate():
            save_client_data_to_reactives(
                reactives_shiny = reactives_with_data_clients, client_level = "Bad_Level", data_category = "Assets", data_value = None,
            )

    def test_personal_info_for_combined_raises_value_error(
        self, reactives_with_data_clients: dict[str, Any],
    ) -> None:
        """Test that personal info for combined raises value error."""
        from shiny import reactive
        ...
        with pytest.raises(Exception_Validation_Input), reactive.isolate():
            save_client_data_to_reactives(
                reactives_shiny = reactives_with_data_clients, client_level = "Clients_Combined", data_category = "Personal_Info", data_value = None,
            )

    def test_non_dataframe_data_value_raises_value_error(
        self, reactives_with_data_clients: dict[str, Any],
    ) -> None:
        """Test that non dataframe data value raises value error."""
        from shiny import reactive
        ...
        with pytest.raises(Exception_Validation_Input), reactive.isolate():
            save_client_data_to_reactives(
                reactives_shiny = reactives_with_data_clients,
                client_level = "Client_Primary",
                data_category = "Assets",
                data_value = {"not": "a dataframe"},  # type: ignore[arg-type]
            )
    ...
    def test_save_returns_reactives_dict(
        ...
        """Test that save returns reactives dict."""
        from shiny import reactive
        ...
        with reactive.isolate():
            result = save_client_data_to_reactives(
                reactives_shiny = reactives_with_data_clients, client_level = "Client_Primary", data_category = "Assets", data_value = None,
            )
...
class TestConvenienceClientWrappers:
    ...
    def test_save_personal_info_primary_roundtrip(
        ...
        """Test that save personal info primary roundtrip."""
        import polars as pl
        ...
        with reactive.isolate():
            save_client_personal_info_to_reactives(
                reactives_shiny = reactives_with_data_clients, client_level = "Client_Primary", data_personal_info = sample_personal_info_df,
            )
            result = get_client_data_from_reactives(
                reactives_shiny = reactives_with_data_clients, client_level = "Client_Primary", data_category = "Personal_Info",
            )
        assert isinstance(result, pl.DataFrame)
    ...
    def test_save_personal_info_to_combined_raises(
        ...
        """Test that save personal info to combined raises."""
        from shiny import reactive
        ...
        with pytest.raises(Exception_Validation_Input), reactive.isolate():
            save_client_personal_info_to_reactives(
                reactives_shiny = reactives_with_data_clients, client_level = "Clients_Combined", data_personal_info = None,
            )
    ...
    def test_save_assets_to_combined(
        ...
        """Test that save assets to combined."""
        import polars as pl
        ...
        with reactive.isolate():
            save_client_assets_to_reactives(
                reactives_shiny = reactives_with_data_clients, client_level = "Clients_Combined", data_assets = sample_assets_df,
            )
            result = get_client_data_from_reactives(
                reactives_shiny = reactives_with_data_clients, client_level = "Clients_Combined", data_category = "Assets",
            )
        assert isinstance(result, pl.DataFrame)
    ...
    def test_save_goals_roundtrip(
        ...
        """Test that save goals roundtrip."""
        import polars as pl
        ...
        with reactive.isolate():
            save_client_goals_to_reactives(
                reactives_shiny = reactives_with_data_clients, client_level = "Client_Partner", data_goals = sample_goals_df,
            )
            result = get_client_data_from_reactives(
                reactives_shiny = reactives_with_data_clients, client_level = "Client_Partner", data_category = "Goals",
            )
        assert isinstance(result, pl.DataFrame)
    ...
    def test_save_income_roundtrip(
        ...
        """Test that save income roundtrip."""
        import polars as pl
        ...
        with reactive.isolate():
            save_client_income_to_reactives(
                reactives_shiny = reactives_with_data_clients, client_level = "Client_Primary", data_income = sample_income_df,
            )
            result = get_client_data_from_reactives(
                reactives_shiny = reactives_with_data_clients, client_level = "Client_Primary", data_category = "Income",
            )
        assert isinstance(result, pl.DataFrame)
    ...
    def test_save_clients_combined_data(
        ...
        """Test that save clients combined data."""
        import polars as pl
        ...
        with reactive.isolate():
            save_clients_combined_data_to_reactives(
                reactives_shiny = reactives_with_data_clients, data_category = "Income", data_value = sample_income_df,
            )
            result = get_client_data_from_reactives(
                reactives_shiny = reactives_with_data_clients, client_level = "Clients_Combined", data_category = "Income",
            )
        assert isinstance(result, pl.DataFrame)
    ...
    def test_save_clients_combined_personal_info_raises(
        ...
        """Test that save clients combined personal info raises."""
        from shiny import reactive
        ...
        with pytest.raises(Exception_Validation_Input), reactive.isolate():
            save_clients_combined_data_to_reactives(
                reactives_shiny = reactives_with_data_clients, data_category = "Personal_Info", data_value = None,
            )
...
class TestBuildClientInfoJsonFromReactives:
    ...
    def test_returns_dict(self, reactives_with_data_clients: dict[str, Any]) -> None:
        ...
        with reactive.isolate():
            result = build_client_info_json_from_reactives(reactives_shiny = reactives_with_data_clients)
        assert isinstance(result, dict)

    def test_top_level_keys(self, reactives_with_data_clients: dict[str, Any]) -> None:
        """Test that top level keys."""
        from shiny import reactive
    ...
    def test_single_or_couple_value(
        ...
        """Test that single or couple value."""
        from shiny import reactive
    ...
    def test_personal_info_has_primary_and_partner(
        ...
        """Test that personal info has primary and partner."""
        from shiny import reactive
    ...
    def test_assets_has_primary_partner_combined(
        ...
        """Test that assets has primary partner combined."""
        from shiny import reactive
...
class TestValidateDataResultsInReactives:
    ...
    def test_none_returns_none_or_false(self) -> None:
        ...
        result = validate_data_results_in_reactives(reactives_shiny = None)  # type: ignore[arg-type]
        assert result is None or (isinstance(result, tuple) and result[0] is False)
    ...
    def test_missing_data_results_key_returns_false(self) -> None:
        ...
        ok, msg = validate_data_results_in_reactives(reactives_shiny = {"Other": {}})
        assert ok is False
    ...
    def test_valid_structure_returns_true(
        ...
        ok, msg = validate_data_results_in_reactives(reactives_shiny = reactives_with_data_clients)
        assert ok is True
        assert isinstance(msg, str)
    ...
    def test_data_results_not_dict_returns_false(self) -> None:
        ...
        ok, _ = validate_data_results_in_reactives(reactives_shiny = {"Data_Results": "not_a_dict"})
        assert ok is False
    ...
    def test_missing_subtab_key_returns_false(self, missing_key: str) -> None:
        """Test that missing subtab key returns false."""
        from src.dashboard.shiny_utils.utils_reporting import (
        ...
        ok, _msg = validate_data_results_in_reactives(reactives_shiny = {"Data_Results": incomplete})
        assert ok is False
...
class TestValidateResultsSubtabKey:
    ...
    def test_none_returns_none_or_false(self) -> None:
        ...
        result = validate_results_subtab_key(subtab_key = None)  # type: ignore[arg-type]
        assert result is None or (isinstance(result, tuple) and result[0] is False)
    ...
    def test_empty_string_returns_none_or_false(self) -> None:
        ...
        result = validate_results_subtab_key(subtab_key = "")
        assert result is None or (isinstance(result, tuple) and result[0] is False)
    ...
    def test_unknown_key_returns_false(self) -> None:
        ...
        ok, msg = validate_results_subtab_key(subtab_key = "Unknown_Subtab")
        assert ok is False
    ...
    def test_case_sensitive(self) -> None:
        ...
        ok, _ = validate_results_subtab_key(subtab_key = "portfolio_analysis_inputs")
        assert ok is False
...
class TestGetSaveResultsData:
    ...
    def test_initial_value_is_none(
        ...
        """Test that initial value is none."""
        from shiny import reactive
        ...
        with reactive.isolate():
            result = get_results_data_from_reactives(
                reactives_shiny = reactives_with_data_clients, subtab_key = subtab_key,
            )
    ...
    def test_save_and_get_dict_roundtrip(
        ...
        """Test that save and get dict roundtrip."""
        from shiny import reactive
        ...
        with reactive.isolate():
            ...
            result = get_results_data_from_reactives(
                reactives_shiny = reactives_with_data_clients, subtab_key = "Portfolio_Analysis_Outputs",
            )
    ...
    def test_save_dataframe_roundtrip(
        ...
        """Test that save dataframe roundtrip."""
        import polars as pl
        ...
        with reactive.isolate():
            save_results_data_to_reactives(
                ...
                data_value = sample_assets_df,
            )
            result = get_results_data_from_reactives(
                reactives_shiny = reactives_with_data_clients, subtab_key = "Weights_Analysis_Outputs",
            )
        assert isinstance(result, pl.DataFrame)
    ...
    def test_invalid_subtab_key_raises_value_error(
        ...
        """Test that invalid subtab key raises value error."""
        from shiny import reactive
        ...
        with pytest.raises(Exception_Validation_Input), reactive.isolate():
            save_results_data_to_reactives(
                reactives_shiny = reactives_with_data_clients, subtab_key = "Invalid_Key", data_value = None,
            )
    ...
    def test_save_returns_reactives_dict(
        ...
        with reactive.isolate():
            result = save_results_data_to_reactives(
                ...
                data_value = {"distribution_type": "normal"},
            )
        assert isinstance(result, dict)
...
class TestConvenienceResultsWrappers:
    ...
    def test_wrapper_writes_to_correct_key(
        ...
        with reactive.isolate():
            wrapper(reactives_shiny=reactives_with_data_clients, data_value=sentinel)
            result = get_results_data_from_reactives(
                reactives_shiny = reactives_with_data_clients, subtab_key = subtab_key,
            )
...
class TestCoerceResultsValueToJson:
    ...
    def test_none_returns_empty_dict(self) -> None:
        ...
        assert _coerce_results_value_to_json(data = None) == {}
    ...
    def test_dict_returned_as_is(self) -> None:
        ...
        payload = {"a": 1, "b": "hello"}
        assert _coerce_results_value_to_json(data = payload) is payload

    def test_list_wrapped_in_records(self) -> None:
        """Test that list wrapped in records."""
        from src.dashboard.shiny_utils.utils_reporting import _coerce_results_value_to_json

        result = _coerce_results_value_to_json(data = [1, 2, 3])
        assert "records" in result
        assert result["records"] == [1, 2, 3]

    def test_polars_dataframe_wrapped_in_records(self) -> None:
        """Test that polars dataframe wrapped in records."""
        import polars as pl
    ...
    def test_arbitrary_type_produces_value_str(self) -> None:
        """Test that arbitrary type produces value str."""
        from src.dashboard.shiny_utils.utils_reporting import _coerce_results_value_to_json
...
class TestBuildResultsDataJsonFromReactives:
    ...
    def test_returns_empty_dict_when_no_data_stored(
        ...
        """Test that returns empty dict when no data stored."""
        from shiny import reactive
        ...
        with reactive.isolate():
            save_results_data_to_reactives(
                reactives_shiny = reactives_with_data_clients, subtab_key = "Portfolio_Comparison_Inputs", data_value = None,
            )
            result = build_results_data_json_from_reactives(
                reactives_shiny = reactives_with_data_clients, subtab_key = "Portfolio_Comparison_Inputs",
            )
        assert result == {}
    ...
    def test_returns_dict_payload_when_dict_stored(
        ...
        """Test that returns dict payload when dict stored."""
        from shiny import reactive
        ...
        with reactive.isolate():
            ...
            result = build_results_data_json_from_reactives(
                reactives_shiny = reactives_with_data_clients, subtab_key = "Portfolio_Analysis_Inputs",
            )
        assert result == payload
    ...
    def test_invalid_subtab_key_raises_value_error(
        ...
        with pytest.raises(Exception_Validation_Input), reactive.isolate():
            build_results_data_json_from_reactives(
                reactives_shiny = reactives_with_data_clients, subtab_key = "Unknown_Key",
            )
    ...
    def test_polars_df_stored_produces_records_key(
        ...
        """Test that polars df stored produces records key."""
        from shiny import reactive
        ...
        with reactive.isolate():
            ...
            result = build_results_data_json_from_reactives(
                reactives_shiny = reactives_with_data_clients, subtab_key = "Weights_Analysis_Inputs",
            )
        assert "records" in result
        assert isinstance(result["records"], list)
    ...
    def test_all_valid_subtab_keys_return_dict(
        ...
        """Test that all valid subtab keys return dict."""
        from shiny import reactive
...
class TestValidResultsSubtabKeys:
    ...
    def test_all_keys_present_in_data_results(
        ...
        """Test that all keys present in data results."""
        from src.dashboard.shiny_utils.utils_reporting import VALID_RESULTS_SUBTAB_KEYS

...
class TestValidateDataUtilsParameter:
    ...
    def test_none_returns_false(self) -> None:
        ...
        ok, msg = validate_data_utils_parameter(data_utils = None)  # type: ignore[arg-type]
        assert ok is False
    ...
    def test_string_returns_false(self) -> None:
        ...
        ok, _msg = validate_data_utils_parameter(data_utils = "not-a-dict")  # type: ignore[arg-type]
        assert ok is False
    ...
    def test_list_returns_false(self) -> None:
        ...
        ok, _msg = validate_data_utils_parameter(data_utils = [])  # type: ignore[arg-type]
        assert ok is False
    ...
    def test_empty_dict_returns_true(self) -> None:
        ...
        ok, _msg = validate_data_utils_parameter(data_utils = {})
        assert ok is True

    def test_populated_dict_returns_true(self) -> None:
        """Test that populated dict returns true."""
        from src.dashboard.shiny_utils.reactives_shiny import validate_data_utils_parameter

        ok, _msg = validate_data_utils_parameter(data_utils = {"key": "value"})
        assert ok is True
    ...
    def test_return_type_is_tuple(self) -> None:
        ...
        result = validate_data_utils_parameter(data_utils = {})
        assert isinstance(result, tuple)
        assert len(result) == 2

    def test_successful_validation_msg_is_empty_or_ok(self) -> None:
        """Test that successful validation msg is empty or ok."""
        from src.dashboard.shiny_utils.reactives_shiny import validate_data_utils_parameter

        ok, msg = validate_data_utils_parameter(data_utils = _valid_data_utils())
        assert ok is True
        assert isinstance(msg, str)
...
class TestValidateReactivesShinyStructure:
    ...
    def test_none_returns_false(self) -> None:
        ...
        ok, msg = validate_reactives_shiny_structure(reactives_shiny = None)  # type: ignore[arg-type]
        assert ok is False
        assert isinstance(msg, str)
    ...
    def test_non_dict_returns_false(self) -> None:
        ...
        ok, _msg = validate_reactives_shiny_structure(reactives_shiny = [])  # type: ignore[arg-type]
        assert ok is False
    ...
    def test_empty_dict_returns_false(self) -> None:
        ...
        ok, _msg = validate_reactives_shiny_structure(reactives_shiny = {})
        assert ok is False

    def test_missing_one_category_returns_false(self) -> None:
        """Test that missing one category returns false."""
        from src.dashboard.shiny_utils.reactives_shiny import (
            validate_reactives_shiny_structure,
        )
        ...
        ok, _msg = validate_reactives_shiny_structure(reactives_shiny = incomplete)
        assert ok is False

    def test_all_required_categories_returns_true(self) -> None:
        """Test that all required categories returns true."""
        from src.dashboard.shiny_utils.reactives_shiny import (
            validate_reactives_shiny_structure,
        )

        ok, msg = validate_reactives_shiny_structure(reactives_shiny = _valid_skeleton())
        assert ok is True
        assert isinstance(msg, str)
    ...
    def test_each_required_category_individually(self, category: str) -> None:
        ...
        ok, _msg = validate_reactives_shiny_structure(reactives_shiny = skeleton)
        assert ok is False
...
class TestValidateReactiveKeyAccess:
    ...
    def test_none_category_dict_returns_false(self) -> None:
        """Test that none category dict returns false."""
        from src.dashboard.shiny_utils.reactives_shiny import (
            validate_reactive_key_access,
        )

        ok, msg = validate_reactive_key_access(category_dict = None, key_name = "key", category_name = "User_Inputs_Shiny")  # type: ignore[arg-type]
        assert ok is False
        assert isinstance(msg, str)

    def test_non_dict_category_returns_false(self) -> None:
        """Test that non dict category returns false."""
        from src.dashboard.shiny_utils.reactives_shiny import (
            validate_reactive_key_access,
        )

        ok, _msg = validate_reactive_key_access(category_dict = "not_a_dict", key_name = "key", category_name = "User_Inputs_Shiny")  # type: ignore[arg-type]
        assert ok is False

    def test_none_key_name_returns_false(self) -> None:
        """Test that none key name returns false."""
        from src.dashboard.shiny_utils.reactives_shiny import (
            validate_reactive_key_access,
        )

        ok, _msg = validate_reactive_key_access(category_dict = {}, key_name = None, category_name = "User_Inputs_Shiny")  # type: ignore[arg-type]
        assert ok is False

    def test_empty_key_name_returns_false(self) -> None:
        """Test that empty key name returns false."""
        from src.dashboard.shiny_utils.reactives_shiny import (
            validate_reactive_key_access,
        )

        ok, _msg = validate_reactive_key_access(category_dict = {"a": 1}, key_name = "", category_name = "User_Inputs_Shiny")
        assert ok is False
    ...
    def test_missing_key_returns_false(self) -> None:
        ...
        ok, _msg = validate_reactive_key_access(category_dict = {"a": 1}, key_name = "b", category_name = "User_Inputs_Shiny")
        assert ok is False
    ...
    def test_existing_key_returns_true(self) -> None:
        ...
        ok, msg = validate_reactive_key_access(category_dict = {"my_key": 42}, key_name = "my_key", category_name = "Inner_Variables_Shiny")
        assert ok is True
        assert isinstance(msg, str)
...
class TestValidateCategoryName:
    ...
    def test_valid_category_names(self, valid_name: str) -> None:
        """Test that valid category names."""
        from src.dashboard.shiny_utils.reactives_shiny import validate_category_name

        ok, msg = validate_category_name(category_name = valid_name)
        assert ok is True
        assert isinstance(msg, str)
    ...
    def test_none_returns_false(self) -> None:
        ...
        ok, _msg = validate_category_name(category_name = None)  # type: ignore[arg-type]
        assert ok is False

    def test_non_str_returns_false(self) -> None:
        """Test that non str returns false."""
        from src.dashboard.shiny_utils.reactives_shiny import validate_category_name

        ok, _msg = validate_category_name(category_name = 123)  # type: ignore[arg-type]
        assert ok is False
    ...
    def test_empty_string_returns_false(self) -> None:
        ...
        ok, _msg = validate_category_name(category_name = "")
        assert ok is False

    def test_unknown_category_name_returns_false(self) -> None:
        """Test that unknown category name returns false."""
        from src.dashboard.shiny_utils.reactives_shiny import validate_category_name

        ok, _msg = validate_category_name(category_name = "Unknown_Category")
        assert ok is False

    def test_case_sensitive_mismatch_returns_false(self) -> None:
        """Test that case sensitive mismatch returns false."""
        from src.dashboard.shiny_utils.reactives_shiny import validate_category_name

        ok, _msg = validate_category_name(category_name = "user_inputs_shiny")  # lowercase
        assert ok is False
...
class TestSafeGetShinyInputValue:
    ...
    def test_none_input_events_returns_none(self) -> None:
        """Test that none input events returns none."""
        from src.dashboard.shiny_utils.reactives_shiny import safe_get_shiny_input_value

        result = safe_get_shiny_input_value(input_events = None, input_identifier = "some_id")  # type: ignore[arg-type]
    ...
    def test_non_string_identifier_returns_none(self) -> None:
        ...
        result = safe_get_shiny_input_value(input_events = {}, input_identifier = 123)  # type: ignore[arg-type]
        assert result is None
    ...
    def test_empty_identifier_returns_none(self) -> None:
        ...
        result = safe_get_shiny_input_value(input_events = {}, input_identifier = "")
        assert result is None
...
class TestCreateReactiveValueSafely:
    ...
    def test_returns_non_none_for_valid_initial_value(self) -> None:
        """Test that returns non none for valid initial value."""
    ...
    def test_returns_non_none_for_none_initial_value(self) -> None:
        """Test that returns non none for none initial value."""
        from shiny import reactive
    ...
    def test_stored_value_is_readable(self) -> None:
        """Test that stored value is readable."""
        from shiny import reactive

        from src.dashboard.shiny_utils.reactives_shiny import create_reactive_value_safely

        with reactive.isolate():
            rv = create_reactive_value_safely(initial_value = "hello")
            assert rv.get() == "hello"

    def test_stored_list_value_is_readable(self) -> None:
        """Test that stored list value is readable."""
        from shiny import reactive

        from src.dashboard.shiny_utils.reactives_shiny import create_reactive_value_safely

        with reactive.isolate():
            rv = create_reactive_value_safely(initial_value = [1, 2, 3])
            assert rv.get() == [1, 2, 3]
...
class TestInitializeReactivesShiny:
    ...
    def test_raises_value_error_for_none_data_utils(self) -> None:
        """Test that raises value error for none data utils."""
        from shiny import reactive

        from src.dashboard.shiny_utils.reactives_shiny import initialize_reactives_shiny

        with pytest.raises((Exception_Validation_Input, TypeError)), reactive.isolate():
            initialize_reactives_shiny(data_utils = None)  # type: ignore[arg-type]

    def test_raises_value_error_for_non_dict_data_utils(self) -> None:
        """Test that raises value error for non dict data utils."""
        from shiny import reactive

        from src.dashboard.shiny_utils.reactives_shiny import initialize_reactives_shiny

        with pytest.raises((Exception_Validation_Input, TypeError)), reactive.isolate():
            initialize_reactives_shiny(data_utils = "bad")  # type: ignore[arg-type]

    def test_returns_dict_for_valid_data_utils(self, data_utils: dict[str, Any]) -> None:
        """Test that returns dict for valid data utils."""
        from shiny import reactive

        from src.dashboard.shiny_utils.reactives_shiny import initialize_reactives_shiny

        with reactive.isolate():
            result = initialize_reactives_shiny(data_utils = data_utils)
        assert isinstance(result, dict)

    def test_has_all_four_categories(self, data_utils: dict[str, Any]) -> None:
        """Test that has all four categories."""
        from shiny import reactive
    ...
    def test_each_category_is_dict(self, data_utils: dict[str, Any]) -> None:
        """Test that each category is dict."""
        from shiny import reactive
...
class TestSafeGetReactiveValue:
    ...
    def test_none_reactive_returns_default_none(self) -> None:
        """Test that none reactive returns default none."""
        from src.dashboard.shiny_utils.reactives_shiny import safe_get_reactive_value

        result = safe_get_reactive_value(reactive_input = None, default_value=None)
        assert result is None

    def test_none_reactive_returns_custom_default(self) -> None:
        """Test that none reactive returns custom default."""
        from src.dashboard.shiny_utils.reactives_shiny import safe_get_reactive_value
    ...
    def test_valid_reactive_returns_its_value(self) -> None:
        """Test that valid reactive returns its value."""
        from shiny import reactive
        ...
        with reactive.isolate():
            ...
            result = safe_get_reactive_value(reactive_input = rv, default_value=0)
        assert result == 99
...
class TestSafeGetValueFromShinyInputText:
    ...
    def test_none_input_returns_default_str(self) -> None:
        """Test that none input returns default str."""
        from src.dashboard.shiny_utils.reactives_shiny import (
            safe_get_value_from_shiny_input_text,
        )

        result = safe_get_value_from_shiny_input_text(input_reactive_object = None, default_value="default")
        assert result == "default"

    def test_none_input_returns_empty_str_when_default_empty(self) -> None:
        """Test that none input returns empty str when default empty."""
        from src.dashboard.shiny_utils.reactives_shiny import (
            safe_get_value_from_shiny_input_text,
        )

        result = safe_get_value_from_shiny_input_text(input_reactive_object = None, default_value="")
        assert result == ""

    def test_valid_reactive_with_string_value(self) -> None:
        """Test that valid reactive with string value."""
        from shiny import reactive
        ...
        with reactive.isolate():
            ...
            result = safe_get_value_from_shiny_input_text(input_reactive_object = rv, default_value="default")
        assert result == "hello"
...
class TestSafeGetValueFromShinyInputNumeric:
    ...
    def test_none_input_returns_numeric_default(self) -> None:
        """Test that none input returns numeric default."""
        from src.dashboard.shiny_utils.reactives_shiny import (
            safe_get_value_from_shiny_input_numeric,
        )

        result = safe_get_value_from_shiny_input_numeric(input_reactive_object = None, default_value=0.0)
        assert result == 0.0

    def test_none_input_with_non_zero_default(self) -> None:
        """Test that none input with non zero default."""
        from src.dashboard.shiny_utils.reactives_shiny import (
            safe_get_value_from_shiny_input_numeric,
        )

        result = safe_get_value_from_shiny_input_numeric(input_reactive_object = None, default_value=42.5)
        assert result == 42.5

    def test_valid_reactive_returns_numeric_value(self) -> None:
        """Test that valid reactive returns numeric value."""
        from shiny import reactive
        ...
        with reactive.isolate():
            ...
            result = safe_get_value_from_shiny_input_numeric(input_reactive_object = rv, default_value=0.0)
...
class TestGetSetValueFromReactivesShiny:
    ...
    def test_set_existing_key_roundtrip(self, minimal_reactives_shiny: dict[str, Any]) -> None:
        ...
        with reactive.isolate():
            set_value_to_reactives_shiny(
                reactives_shiny = minimal_reactives_shiny,
...
class TestReactiveKeyToInputId:
    ...
    def test_clients_key_produces_correct_input_id(self) -> None:
        """Test that clients key produces correct input id."""
        from src.dashboard.shiny_utils.reactives_shiny import _reactive_key_to_input_id

        result = _reactive_key_to_input_id(
            reactive_key = "Input_Tab_clients_Subtab_clients_Personal_Info_client_Primary_Name",
        )
    ...
    def test_portfolio_comparison_key(self) -> None:
        """Test that portfolio comparison key."""
        from src.dashboard.shiny_utils.reactives_shiny import _reactive_key_to_input_id

        result = _reactive_key_to_input_id(
            reactive_key = "Input_Tab_Portfolios_Subtab_Comparison_Time_Period",
        )
        assert result == "input_ID_tab_portfolios_subtab_comparison_time_period"

    def test_weights_analysis_key(self) -> None:
        """Test that weights analysis key."""
        from src.dashboard.shiny_utils.reactives_shiny import _reactive_key_to_input_id

        result = _reactive_key_to_input_id(
            reactive_key = "Input_Tab_Portfolios_Subtab_Weights_Analysis_Viz_Type",
        )
        assert result == "input_ID_tab_portfolios_subtab_weights_analysis_viz_type"

    def test_skfolio_key_method1(self) -> None:
        """Test that skfolio key method1."""
        from src.dashboard.shiny_utils.reactives_shiny import _reactive_key_to_input_id

        result = _reactive_key_to_input_id(
            reactive_key = "Input_Tab_Portfolios_Subtab_Skfolio_Method1_Category",
        )
        assert result == "input_ID_tab_portfolios_subtab_skfolio_method1_category"

    def test_simulation_key(self) -> None:
        """Test that simulation key."""
        from src.dashboard.shiny_utils.reactives_shiny import _reactive_key_to_input_id

        result = _reactive_key_to_input_id(
            reactive_key = "Input_Tab_Results_Subtab_Simulation_Distribution_Type",
        )
        assert result == "input_ID_tab_results_subtab_simulation_distribution_type"

    def test_bad_prefix_raises_value_error(self) -> None:
        """Test that bad prefix raises value error."""
        from src.dashboard.shiny_utils.reactives_shiny import _reactive_key_to_input_id

        with pytest.raises(Exception_Validation_Input, match="Input_Tab_"):
            _reactive_key_to_input_id(reactive_key = "Some_Arbitrary_Key_Without_Correct_Prefix")
    ...
    def test_empty_string_raises_value_error(self) -> None:
        ...
        with pytest.raises(Exception_Validation_Input):
            _reactive_key_to_input_id(reactive_key = "")

    def test_preserves_lowercase_suffix(self) -> None:
        """Test that preserves lowercase suffix."""
        from src.dashboard.shiny_utils.reactives_shiny import _reactive_key_to_input_id

        result = _reactive_key_to_input_id(reactive_key = "Input_Tab_ABC_Subtab_XYZ")
...
class TestNewUserInputsShinyKeysPresent:
    ...
    def test_weights_analysis_key_present(
        ...
        """Test that weights analysis key present."""
    ...
    def test_simulation_key_present(
        ...
        """Test that simulation key present."""
        assert key in minimal_reactives_shiny["User_Inputs_Shiny"], (
...
class TestPortfolioSubtabResultsKeyMap:
    ...
    def test_map_contains_five_entries(self) -> None:
        """Test that map contains five entries."""
    ...
    def test_expected_base_key_present(self, base_key: str) -> None:
        """Test that expected base key present."""
        from src.dashboard.shiny_utils.reactives_shiny import _PORTFOLIO_SUBTAB_RESULTS_KEY_MAP
...
class TestRegisterClientInputObserversValidation:
    ...
    def test_none_input_raises_value_error(
        ...
        with pytest.raises(Exception_Validation_Input, match="input cannot be None"), reactive.isolate():
            register_client_input_observers(input = None, reactives_shiny = minimal_reactives_shiny)  # type: ignore[arg-type]

    def test_invalid_reactives_structure_raises_value_error(self) -> None:
        """Test that invalid reactives structure raises value error."""
        from shiny import reactive
        ...
        with pytest.raises(Exception_Validation_Input), reactive.isolate():
            register_client_input_observers(input = _FakeInput(), reactives_shiny = {"wrong": {}})
    ...
    def test_valid_call_does_not_raise(
        ...
        with reactive.isolate():
            # Must not raise
            register_client_input_observers(input = _FakeInput(), reactives_shiny = minimal_reactives_shiny)
...
class TestRegisterPortfolioResultsObserversValidation:
    ...
    def test_none_input_raises_value_error(
        ...
        with pytest.raises(Exception_Validation_Input), reactive.isolate():
            register_portfolio_results_observers(input = None, reactives_shiny = minimal_reactives_shiny)  # type: ignore[arg-type]
    ...
    def test_invalid_reactives_raises_value_error(self) -> None:
        ...
        with pytest.raises(Exception_Validation_Input), reactive.isolate():
            register_portfolio_results_observers(input = _FakeInput(), reactives_shiny = {"bad": "structure"})

    def test_valid_call_does_not_raise(
        self, minimal_reactives_shiny: dict[str, Any],
    ) -> None:
        """Test that valid call does not raise."""
        from shiny import reactive
        ...
        with reactive.isolate():
            register_portfolio_results_observers(input = _FakeInput(), reactives_shiny = minimal_reactives_shiny)
...
class TestInitializeReactivesShinyExtended:
    ...
    def test_has_data_clients_category(self, minimal_reactives_shiny: dict[str, Any]) -> None:
        """Test that has data clients category."""
    ...
    def test_data_results_has_all_ten_subtab_keys(
        ...
        """Test that data results has all ten subtab keys."""

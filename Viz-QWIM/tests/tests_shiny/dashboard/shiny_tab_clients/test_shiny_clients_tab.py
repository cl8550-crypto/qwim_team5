...
class Test_Summary_Subtab_UI:
    ...
    def test_summary_subtab_renders_content(
        ...
        # Wait for Shiny to render the summary table.
        page.wait_for_selector(
            "table, .shiny-html-output, .gt_table, [id*='summary'] .shiny-bound-output",
            timeout=10_000,
            state="attached",
        )
    ...
    def test_changing_name_updates_summary(
        ...
        # Wait for the input value to propagate through the reactive graph.
        page.wait_for_function(
            f"document.querySelector('#{_ID_PRIMARY_NAME}')?.value === '{test_name}'",
            timeout=5_000,
        )

        # Navigate to Summary
        _click_subtab(page, _SEL_SUBTAB_SUMMARY)
        # Wait for summary content to update with the new name.
        page.wait_for_selector(
            f"text={test_name}",
            timeout=10_000,
            state="attached",

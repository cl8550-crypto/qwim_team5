...
class keywords_shiny_tab_portfolios:
    ...
    @keyword("Module Source Should Include Text")
    def module_source_should_include_text(self, module_name: str, expected_text: str) -> None:
        """Assert that a module source file contains the expected text fragment.

        Args:
            module_name: Short module name.
            expected_text: Required source fragment.
        """
        source_text = self._get_source(module_name)
        assert expected_text in source_text, (
            f"Expected '{expected_text}' in source for module '{module_name}'"
        )
        _logger.info(f"Source for '{module_name}' contains '{expected_text}' — OK")

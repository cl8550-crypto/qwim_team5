...
def validate_portfolio_data_with_none() -> tuple:
    """Call validate_portfolio_data(portfolio_dataframe=None) and return (is_valid, message)."""
    _assert_modules_available()
    return validate_portfolio_data(portfolio_dataframe=None)


def validate_portfolio_data_with_empty_df() -> tuple:
    """Call validate_portfolio_data with an empty DataFrame."""
    _assert_modules_available()
    return validate_portfolio_data(portfolio_dataframe=pd.DataFrame())
...
def validate_portfolio_data_without_date_column() -> tuple:
    ...
    return validate_portfolio_data(portfolio_dataframe=df)
...
def validate_negative_financial_amount_raises() -> str:
    ...
    _assert_modules_available()
    try:
        validate_financial_amount(amount_value = -100.0)
        return ""
...
def validate_valid_age(age: int = 45) -> int:
    ...
    _assert_modules_available()
    return validate_age_range(age_value = age, minimum_age=18, maximum_age=100, age_type_description="age")
...
def check_attribute_error_is_silent() -> bool:
    ...
    _assert_modules_available()
    return is_silent_exception(exception = AttributeError("reactive not ready"))


def check_value_error_is_not_silent() -> bool:
    """Return is_silent_exception(ValueError(...))."""
    _assert_modules_available()
    return is_silent_exception(exception = ValueError("bad value"))

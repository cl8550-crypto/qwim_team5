...
def load_raw_input_data() -> dict:
    ...
    """
    _require_imports()
    return get_input_data_raw(project_dir = _PROJECT_ROOT)
...
def validate_valid_portfolio_dataframe() -> tuple:
    ...
    return validate_portfolio_data(data_portfolio=df)


def validate_none_portfolio_data() -> tuple:
    """Call validate_portfolio_data(portfolio_dataframe=None).

    Returns: (False, error_message) tuple
    """
    _require_imports()
    return validate_portfolio_data(data_portfolio=None)
...
def calculate_returns_from_valid_portfolio_df() -> "pl.Series":
    ...
    return calculate_portfolio_returns(data_portfolio = df)
...
def process_small_dataframe_for_plot() -> "pl.DataFrame":
    ...
    return process_data_for_plot_tab_results(data_frame = df, date_column="date", max_points=5000)
...
def process_large_dataframe_returns_fewer_rows() -> None:
    ...
    result = process_data_for_plot_tab_results(data_frame = df, date_column="date", max_points=max_pts)
    if len(result) >= rows:
        raise AssertionError(
...
def normalize_with_none_method_returns_unchanged() -> "pl.DataFrame":
    ...
    return normalize_data_tab_results(data_frame = df, method="none")
...
def normalize_with_z_score_returns_dataframe() -> "pl.DataFrame":
    ...
    _require_imports()
    df = _make_tab_results_df(rows=20)
    return normalize_data_tab_results(data_frame = df, method="z_score")
...
def transform_with_none_returns_dataframe_unchanged() -> "pl.DataFrame":
    ...
    _require_imports()
    df = _make_tab_results_df(rows=10)
    return transform_data_tab_results(data_frame = df, transformation="none")
...
def transform_with_percent_change_adds_pct_columns() -> "pl.DataFrame":
    ...
    _require_imports()
    df = _make_tab_results_df(rows=10)
    return transform_data_tab_results(data_frame = df, transformation="percent_change")
...
def transform_with_cumulative_adds_cum_columns() -> "pl.DataFrame":
    ...
    _require_imports()
    df = _make_tab_results_df(rows=10)
    return transform_data_tab_results(data_frame = df, transformation="cumulative")

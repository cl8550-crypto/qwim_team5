...
def format_currency(amount: float | str) -> str:
    ...
    """
    _require_imports()
    return format_currency_value(amount = float(amount))
...
def extract_numeric_from_currency(currency_string: str) -> float:
    ...
    """
    _require_imports()
    return extract_numeric_from_currency_string(currency_string = currency_string)
...
def currency_round_trip_should_preserve_value(amount: float | str) -> None:
    ...
    formatted = format_currency_value(amount = amount_float)
...
def extract_from_none_alternative_should_return_zero() -> None:
    ...
    """
    _require_imports()
    result = extract_numeric_from_currency_string(currency_string = 0)  # type: ignore[arg-type]
    if abs(result) > 1.0e-9:
        raise AssertionError(

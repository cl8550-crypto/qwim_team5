...
try:
    ...
    from src.dashboard.reporting import compile_typst_report, validate_polars_DF
    import polars as pl
...
def validate_polars_df_with_valid_dataframe() -> Any:
    ...
    result = validate_polars_DF(polars_DF = df)
    if result is False:
        raise AssertionError(
...
def validate_polars_df_with_none_input() -> None:
    ...
    _require_imports()
    try:
        validate_polars_DF(polars_DF = None)  # type: ignore[arg-type]
...
def compile_typst_report_with_missing_template_should_fail() -> None:
    """Verify compile_typst_report returns a controlled missing-template failure."""
    _require_imports()
    with tempfile.TemporaryDirectory(prefix="qwim_typst_robot_") as temp_dir_text:
        temp_dir_path = Path(temp_dir_text)
        missing_template_path = temp_dir_path / "missing.typ"
        output_pdf_path = temp_dir_path / "report.pdf"

        success_flag, status_message = compile_typst_report(
            typ_template_path = missing_template_path,
            output_pdf_path = output_pdf_path,
        )

        if success_flag:
            raise AssertionError(
                "compile_typst_report unexpectedly succeeded for a missing template"
            )
        if "not found" not in status_message.lower():
            raise AssertionError(
                f"Expected missing-template message, got: {status_message}"
            )

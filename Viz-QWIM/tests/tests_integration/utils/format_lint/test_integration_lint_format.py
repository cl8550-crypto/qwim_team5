...
class Test_Run_Command_Integration:
    ...
    def test_run_python_command_returns_zero(self):
        """run_command executes a simple Python command successfully."""
    ...
    def test_run_nonexistent_command_returns_nonzero(self):
        ...
        result = run_command(command = ["this_command_does_not_exist_xyz_abc_123"])
        assert result != 0
    ...
    def test_run_python_version_command_returns_zero(self):
        ...
        result = run_command(command = [python_exe, "--version"])
        assert result == 0
...
class Test_Lint_And_Format_File_Integration:
    ...
    def test_lint_clean_file_succeeds(self, tmp_path):
        ...
        result = lint_and_format_file(file_path = py_file)
    ...
    def test_lint_non_python_file_skipped(self, tmp_path):
        ...
        result = lint_and_format_file(file_path = txt_file)
    ...
    def test_lint_nonexistent_file_returns_nonzero(self, tmp_path):
        ...
        result = lint_and_format_file(file_path = missing_file)
        assert result != 0
    ...
    def test_lint_file_with_path_object(self, tmp_path):
        ...
        result = lint_and_format_file(file_path = Path(py_file))
        assert isinstance(result, int)
...
class Test_Lint_And_Format_Directory_Integration:
    ...
    def test_lint_empty_directory_returns_integer(self, tmp_path):
        ...
        result = lint_and_format_directory(directory_path = tmp_path)
        assert isinstance(result, int)
    ...
    def test_lint_nonexistent_directory_returns_nonzero(self, tmp_path):
        ...
        result = lint_and_format_directory(directory_path = missing_dir)
        assert result != 0
...
class Test_Full_Pipeline_Integration:
    ...
    def test_file_lint_consistent_with_directory_lint(self, tmp_path):
        ...
        file_result = lint_and_format_file(file_path = py_file)
        dir_result = lint_and_format_directory(directory_path = tmp_path)
    ...
    def test_run_command_followed_by_lint_file(self, tmp_path):
        """run_command executes Python, then lint_and_format_file processes a file."""
        command_result = run_command(command = [sys.executable, "-c", "print('pipeline-test')"])
        assert command_result == 0

        py_file = tmp_path / "pipeline.py"
        py_file.write_text('"""Pipeline module."""\n\nB = 2\n', encoding="utf-8")
        lint_result = lint_and_format_file(file_path = py_file)

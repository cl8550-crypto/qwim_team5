...
class Test_Bond_Accrued_Interest_Calculation:
    ...
    def test_annual_coupon_30_360_full_year_accrual(self):
        ...
        year_frac = calc.calc_year_fraction(date_start = date(2023, 1, 1), date_end = date(2024, 1, 1))
    ...
    def test_semiannual_coupon_act_365_half_year_accrual(self):
        ...
        year_frac = calc.calc_year_fraction(date_start = date(2024, 1, 1), date_end = date(2024, 7, 1))
        # 182 / 365 ≈ 0.4986
        accrued = coupon_rate * year_frac
    ...
    def test_act_act_one_year_yields_unity_regardless_of_leap_status(self):
        ...
        calc = get_daycount_calculator(convention = Daycount_Convention.ACTUAL_ACTUAL)
        for year_start in range(2020, 2027):
            d_start = date(year_start, 1, 1)
            d_end = date(year_start + 1, 1, 1)
            year_frac = calc.calc_year_fraction(date_start = d_start, date_end = d_end)
    ...
    def test_cross_year_boundary_act_act_additive(self):
        ...
        calc = get_daycount_calculator(convention = Daycount_Convention.ACTUAL_ACTUAL)
        year_frac = calc.calc_year_fraction(date_start = date(2023, 1, 1), date_end = date(2025, 1, 1))
    ...
    def test_act_360_produces_greater_than_act_365_same_period(self):
        ...
        calc_360 = get_daycount_calculator(convention = Daycount_Convention.ACTUAL_360)
        calc_365 = get_daycount_calculator(convention = Daycount_Convention.ACTUAL_365)
        d_start = date(2024, 3, 1)
        d_end = date(2024, 9, 1)
        frac_360 = calc_360.calc_year_fraction(date_start = d_start, date_end = d_end)
        frac_365 = calc_365.calc_year_fraction(date_start = d_start, date_end = d_end)
    ...
    def test_30_360_vs_30_365_same_numerator_different_denominator(self):
        ...
        calc_365 = get_daycount_calculator(convention = Daycount_Convention.THIRTY_365)
...
class Test_Multi_Period_Consistency:
    ...
    def test_two_half_years_sum_to_full_year_act_365(self):
        ...
        h1 = calc.calc_year_fraction(date_start = d0, date_end = d1)
        h2 = calc.calc_year_fraction(date_start = d1, date_end = d2)
        full = calc.calc_year_fraction(date_start = d0, date_end = d2)
    ...
    def test_quarterly_periods_sum_to_annual_30_360(self):
        ...
        total = sum(calc.calc_year_fraction(date_start=q[0], date_end=q[1]) for q in quarters)
        assert total == pytest.approx(1.0, rel=1e-9)

    @pytest.mark.integration()
    def test_daily_sum_matches_act_360_known_period(self):
        """Sum of 90 daily ACT/360 fractions = 90 × (1/360) = 0.25."""
        calc = get_daycount_calculator(convention = Daycount_Convention.ACTUAL_360)
        # 2025-01-01 → 2025-04-01 = 90 days
        d_start = date(2025, 1, 1)
        d_end = date(2025, 4, 1)
        full_fraction = calc.calc_year_fraction(date_start = d_start, date_end = d_end)
    ...
    def test_all_conventions_agree_on_zero_for_same_date(self):
        ...
        for convention in Daycount_Convention:
            ...
            assert calc.calc_year_fraction(date_start = d, date_end = d) == 0.0, (
...
class Test_Factory_Robustness:
    ...
    def test_factory_produces_independent_calculator_instances(self):
        ...
        calc1 = get_daycount_calculator(convention = Daycount_Convention.THIRTY_360)
        calc2 = get_daycount_calculator(convention = Daycount_Convention.THIRTY_360)
    ...
    def test_factory_round_trip_all_conventions(self):
        ...
        for convention in Daycount_Convention:
            calc = get_daycount_calculator(convention = convention)
            result = calc.calc_year_fraction(date_start = d_start, date_end = d_end)
...
class Test_Forward_Rate_Consistency:
    ...
    def test_act_act_preserves_per_diem_interpretation(self):
        ...
        calc = get_daycount_calculator(convention = Daycount_Convention.ACTUAL_ACTUAL)
        frac = calc.calc_year_fraction(date_start = date(2024, 1, 1), date_end = date(2024, 1, 2))

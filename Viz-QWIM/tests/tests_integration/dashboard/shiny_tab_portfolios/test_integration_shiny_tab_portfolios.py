...
class Test_Input_ID_Naming_Conventions:
    ...
    _ANALYSIS_OUTPUT_IDS: ClassVar[list[str]] = [
    ...
    _WEIGHTS_OUTPUT_IDS: ClassVar[list[str]] = [
    ...
    def test_analysis_output_ids_follow_convention(self) -> None:
        """Analysis output IDs keep the public hierarchical naming contract."""
        prefix = "output_ID_tab_portfolios_subtab_portfolios_analysis_"
        for output_id in self._ANALYSIS_OUTPUT_IDS:
            assert output_id.startswith(prefix), (
                f"Output ID '{output_id}' must start with '{prefix}'"
            )

    def Test_Weights_Output_IDs_Follow_Convention(self) -> None:
        """Weights output IDs keep the public hierarchical naming contract."""
        prefix = "output_ID_tab_portfolios_subtab_weights_analysis_"
        for output_id in self._WEIGHTS_OUTPUT_IDS:
            assert output_id.startswith(prefix), (
                f"Output ID '{output_id}' must start with '{prefix}'"
            )
...
@pytest.mark.integration()
@pytestmark_modules
class Test_Analysis_Defaults_And_Output_Surface:
    """Analysis-specific defaults and public output IDs remain stable."""

    def test_analysis_time_period_default_is_1y(self) -> None:
        """The public analysis time-period control defaults to 1y."""
        import src.dashboard.shiny_tab_portfolios.subtab_portfolios_analysis as analysis_module

        source_text = Path(analysis_module.__file__).read_text(encoding="utf-8")
        start_idx = source_text.index(
            "input_ID_tab_portfolios_subtab_portfolios_analysis_time_period",
        )
        context_block = source_text[start_idx : start_idx + 500]

        assert 'selected="1y"' in context_block


@pytest.mark.integration()
@pytestmark_modules
class Class_Test_Weights_Defaults_And_Output_Surface:
    """Weights-specific defaults, outputs, and visualization options remain stable."""

    def Test_Weights_Time_Period_Default_Is_5y(self) -> None:
        """The public weights time-period control defaults to 5y."""
        import src.dashboard.shiny_tab_portfolios.subtab_weights_analysis as weights_module

        source_text = Path(weights_module.__file__).read_text(encoding="utf-8")
        start_idx = source_text.index(
            "input_ID_tab_portfolios_subtab_weights_analysis_time_period",
        )
        context_block = source_text[start_idx : start_idx + 500]

        assert 'selected="5y"' in context_block

    def Test_Weights_Source_Declares_Required_Output_Identifiers(self) -> None:
        """The public weights facade keeps the required output identifiers."""
        import src.dashboard.shiny_tab_portfolios.subtab_weights_analysis as weights_module
        ...
        for output_id in required_output_ids:
            assert output_id in source_text

    def Test_Weights_Source_Declares_Bar_And_Heatmap_Options(self) -> None:
        """The public weights visualization selector keeps the bar and heatmap options."""
        import src.dashboard.shiny_tab_portfolios.subtab_weights_analysis as weights_module

        source_text = Path(weights_module.__file__).read_text(encoding="utf-8")

        assert '"bar": "Stacked Bar Chart"' in source_text
        assert '"heatmap": "Heatmap"' in source_text

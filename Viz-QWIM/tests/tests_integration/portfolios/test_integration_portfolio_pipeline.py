...
class Test_Portfolio_Construction_Integration:
    ...
    def test_load_weights_and_construct_portfolio(self) -> None:
        ...
        weights_loaded = portfolio.get_portfolio_weights()
        assert weights_loaded.schema["Date"] == pl.Date, (
            "Weights Date column must be pl.Date after portfolio_QWIM coercion"
        )
    ...
    def test_portfolio_get_portfolio_weights_returns_dataframe(
        ...
        assert weights.schema["Date"] == pl.Date, "Weights Date column must be pl.Date after coercion"

    @pytest.mark.integration()
    def Test_Portfolio_Weights_Date_Column_Is_Pl_Date(
        self, built_portfolio: "Portfolio_QWIM"
    ) -> None:
        """portfolio_QWIM always coerces the weights Date column to pl.Date."""
        _logger.debug("Testing Date dtype coercion in portfolio_QWIM weights")

        weights = built_portfolio.get_portfolio_weights()

        assert weights.schema["Date"] == pl.Date, (
            "portfolio_QWIM must coerce Date to pl.Date via m_coerce_dataframe"
        )
...
class Test_Portfolio_Value_Calculation_Integration:
    ...
    def test_calculate_portfolio_values_returns_dataframe(
        ...
        assert result.schema["Date"] == pl.Date, "Result Date column must be typed as pl.Date"
    ...
    @pytest.mark.integration()
    def Test_Date_Column_Dtype_Is_Pl_Date(
        self,
        calculated_portfolio_values: pl.DataFrame,
    ) -> None:
        """calculate_portfolio_values returns Date column typed as pl.Date."""
        _logger.debug("Testing Date column dtype in calculate_portfolio_values output")

        assert calculated_portfolio_values.schema["Date"] == pl.Date, (
            "Date column must be pl.Date, not a string type"
        )

    @pytest.mark.integration()
    def Test_Date_Dtype_Normalisation_With_String_Price_Data(
        self,
        built_portfolio: "Portfolio_QWIM",
    ) -> None:
        """calculate_portfolio_values normalises string-typed price Date to pl.Date."""
        _logger.debug("Testing Date normalisation from pl.Utf8 input in price data")

        string_price_data = pl.DataFrame(
            {
                "Date": [
        ...
        result = calculate_portfolio_values(
            portfolio_obj=built_portfolio,
            price_data=string_price_data,
            initial_value=100.0,
        )

        assert isinstance(result, pl.DataFrame), "Result must be a Polars DataFrame"
        assert result.height > 0, "Result must have rows"
        assert result.schema["Date"] == pl.Date, (
            "Date column must be normalised to pl.Date even when price data uses strings"
        )
...
class Test_Benchmark_Creation_Integration:
    ...
    def test_benchmark_has_same_shape_as_portfolio_values(
        ...
        benchmark = create_benchmark_portfolio_values(portfolio_values = calculated_portfolio_values)
    ...
    def test_benchmark_has_same_date_column(
        ...
        assert benchmark.schema["Date"] == calculated_portfolio_values.schema["Date"], (
            "Benchmark Date dtype must match portfolio values Date dtype"
        )
    ...
    @pytest.mark.integration()
    def Test_Benchmark_Date_Dtype_Is_Pl_Date(
        self,
        calculated_portfolio_values: pl.DataFrame,
    ) -> None:
        """create_benchmark_portfolio_values preserves pl.Date dtype on the Date column."""
        _logger.debug("Testing Date dtype preservation through benchmark creation")

        benchmark = create_benchmark_portfolio_values(portfolio_values = calculated_portfolio_values)

        assert benchmark.schema["Date"] == pl.Date, (
            "Benchmark Date column must be pl.Date matching source portfolio values"
        )
...
class Test_Sample_Portfolio_Full_Pipeline_Integration:
    ...
    @pytest.mark.integration()
    def Test_Full_Pipeline_Date_Types_Are_Pl_Date(
        self, full_pipeline_outputs: tuple
    ) -> None:
        """All Date columns produced by the full pipeline are typed as pl.Date."""
        _logger.debug("Testing pl.Date dtype consistency across full pipeline outputs")

        _portfolio, etf_data, portfolio_values, benchmark_values = full_pipeline_outputs

        assert etf_data.schema["Date"] == pl.Date, "ETF data Date must be pl.Date"
        assert portfolio_values.schema["Date"] == pl.Date, (
            "Portfolio values Date must be pl.Date"
        )
        assert benchmark_values.schema["Date"] == pl.Date, (
            "Benchmark values Date must be pl.Date"
        )

    @pytest.mark.integration()
    def Test_Full_Pipeline_No_Nulls_In_Portfolio_Values(
        self, full_pipeline_outputs: tuple
    ) -> None:
        """Portfolio values DataFrame from the full pipeline contains no null values."""
        _logger.debug("Testing absence of nulls in full pipeline portfolio values")

        _portfolio, _etf, portfolio_values, _bench = full_pipeline_outputs

        for item_col in portfolio_values.columns:
            null_count = portfolio_values[item_col].null_count()
            assert null_count == 0, f"Column '{item_col}' must have no nulls, found {null_count}"

    @pytest.mark.integration()
    def Test_Full_Pipeline_All_Portfolio_Values_Positive(
        self, full_pipeline_outputs: tuple
    ) -> None:
        """All portfolio values from the full pipeline are strictly positive."""
        _logger.debug("Testing positivity of all portfolio values in full pipeline")

        _portfolio, _etf, portfolio_values, _bench = full_pipeline_outputs

        val_col = [c for c in portfolio_values.columns if c != "Date"][0]
        assert portfolio_values[val_col].min() > 0, (
            "All portfolio values must be strictly positive"
        )

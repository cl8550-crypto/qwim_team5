...
def _generate_constant_baselines() -> None:
    ...
    df_summary = model_hist.get_summary_statistics(df_predict = df_predict_hist)
...
def _generate_standard_baselines() -> None:
    ...
    model_annual.fit(data = annual_data)
    ...
    model_monthly.fit(data = monthly_data)
    ...
    for t_yr in horizons:
        mean_val = model_annual.get_analytical_mean(t_years=t_yr)
        var_val = model_annual.get_analytical_variance(t_years = t_yr)

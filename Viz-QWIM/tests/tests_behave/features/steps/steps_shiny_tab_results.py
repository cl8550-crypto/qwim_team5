...
def _read_results_source(name: str) -> str:
    """Return the source text for a shiny_tab_results submodule."""
...
@then('the "{module_name}" source should contain "{expected_text}"')
def step_results_source_contains_text(
    context: Any,
    module_name: str,
    expected_text: str,
) -> None:
    """Assert that a Results-tab source file contains a required fragment."""
    del context
    source = _read_results_source(module_name)

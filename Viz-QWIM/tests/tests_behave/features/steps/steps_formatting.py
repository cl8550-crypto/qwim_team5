...
def step_format_currency(context, amount: float) -> None:
    ...
    _require_imports()
    context.formatted_value = format_currency_value(amount = float(amount))
...
def step_result_starts_with_dollar(context) -> None:
    """Step result starts with dollar."""
...
def step_extract_numeric(context, currency_str: str) -> None:
    ...
    _require_imports()
    context.extracted_value = extract_numeric_from_currency_string(currency_string = currency_str)
...
def step_extract_non_string(context) -> None:
    """Step extract non string."""
    _require_imports()
    context.extracted_value = extract_numeric_from_currency_string(currency_string = 0)  # type: ignore[arg-type]
...
def step_currency_round_trip(context, amount: float) -> None:
    """Step currency round trip."""
    _require_imports()
    context.round_trip_amount = float(amount)
    formatted = format_currency_value(amount = float(amount))
    context.extracted_value = extract_numeric_from_currency_string(currency_string = formatted)


@then(u'the round-trip should preserve the value')
def step_round_trip_preserved(context) -> None:
    """Step round trip preserved."""
...
def step_format_percentage_no_multiply(context, value: float) -> None:
    """Step format percentage no multiply."""
    _require_imports()
    context.formatted_value = format_enhanced_percentage_display(
        value = float(value),
        decimal_places=2,
...
def step_format_number_with_places(context, value: float, places: int) -> None:
    """Step format number with places."""
    _require_imports()
    context.formatted_value = format_enhanced_number_display(
        number = float(value),

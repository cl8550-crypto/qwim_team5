...
Run (from project root):
    behave tests/tests_behave/features/ --tags=shiny_tab_clients
...
def _import_subtab(name: str) -> Any:
    """Return a shiny_tab_clients submodule by short name."""
    try:
...
def _import_utils_tab_clients() -> Any:
    """Return the utils_tab_clients module."""
    try:
...
def step_modules_importable(context: Any) -> None:
    """Verify all shiny_tab_clients modules can be imported."""
    names = [
        "tab_clients",
...
def step_ids_start_with_prefix(context: Any, prefix: str) -> None:
    """The prefix must appear at least once in module source."""
...
def step_inspect_primary_name_default(context: Any) -> None:
    """Locate the default primary name value in source."""
...
def step_format_amount_currency(context: Any, amount: str) -> None:
    ...
    context.format_result = context.utils.format_currency_display(amount_value = numeric)
...
def step_format_none_currency(context: Any) -> None:
    ...
    context.format_result = context.utils.format_currency_display(amount_value = None)
...
def step_validate_age(context: Any, age: int) -> None:
    ...
    try:
        context.age_validation_result = context.utils.validate_age_range(
            age_value = age, min_age=18, max_age=100
        )
...
def step_validate_financial_amount(context: Any, amount: str) -> None:
    ...
    numeric = float(amount)
    try:
        context.financial_validation_result = context.utils.validate_financial_amount(
...
def step_validation_passes(context: Any) -> None:
    ...
    if hasattr(context, "age_validation_raised"):
        ...
        assert result is True or result is None or (
            isinstance(result, int | float) and result >= 0
        )
    else:
        assert not context.financial_validation_raised, (
            f"Unexpected exception: "
            f"{getattr(context, 'financial_validation_exception', '')}"
        )
...
def step_validation_fails(context: Any) -> None:
    ...
    if hasattr(context, "age_validation_raised"):
        if not context.age_validation_raised:
            assert (
                context.age_validation_result is False
                or context.age_validation_result is None
            )
    else:
        if not context.financial_validation_raised:
            assert (
                context.financial_validation_result is False
                or context.financial_validation_result is None
            )
...
def step_inspect_server_return_keys(context: Any) -> None:
    ...
    context.server_source = inspect.getsource(context.module)
...
def step_inspect_ui_navset(context: Any) -> None:
    ...
    context.ui_source = inspect.getsource(context.module)

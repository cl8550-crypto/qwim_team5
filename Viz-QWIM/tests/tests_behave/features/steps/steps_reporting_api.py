...
def step_call_validate_with_dataframe(context: Any) -> None:
    ...
    _require_imports()
    try:
        context.last_result = validate_polars_DF(polars_DF = context.test_input)
        context.last_exception = None
...
def step_call_validate_with_none(context: Any) -> None:
    ...
    _require_imports()
    try:
        context.last_result = validate_polars_DF(polars_DF = context.test_input)  # type: ignore[arg-type]
        context.last_exception = None
...
@given("a missing Typst template path and an output PDF path")
def step_given_missing_typst_template(context: Any) -> None:
    """Create a missing Typst-template path for failure-path testing."""
    _require_imports()
    context.typst_temp_dir_handle = tempfile.TemporaryDirectory(prefix="qwim_typst_behave_")
    temp_dir_path = Path(context.typst_temp_dir_handle.name)
    context.test_input = temp_dir_path / "missing.typ"
    context.output_pdf_path = temp_dir_path / "report.pdf"


@when("I call compile_typst_report with the missing template")
def step_call_compile_typst_with_missing_template(context: Any) -> None:
    """Call compile_typst_report with a missing template path."""
    _require_imports()
    try:
        context.last_result = compile_typst_report(
            typ_template_path = context.test_input,
...
@then("compile_typst_report should return failure mentioning missing input")
def step_compile_typst_missing_template_failure(context: Any) -> None:
    """Verify missing-template failures are returned as controlled tuples."""
    if context.last_exception is not None:
        raise AssertionError(
            f"compile_typst_report raised an unexpected exception: {context.last_exception}"
        ) from context.last_exception

    assert isinstance(context.last_result, tuple)
    assert len(context.last_result) == 2

    success_flag, status_message = context.last_result

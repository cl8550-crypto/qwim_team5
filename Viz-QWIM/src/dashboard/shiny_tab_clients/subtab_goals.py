"""Goals subtab UI and server logic with source-visible public Shiny hooks."""
...
# ``_subtab_goals_helpers.py``; the function is exposed here so test code
# can target the public facade without reaching into a private module.
from src.dashboard.shiny_tab_clients._subtab_goals_helpers import (  # noqa: E402
    _coerce_goals_worksheet_amount_int_or_default,
)


@module.ui
def subtab_clients_goals_ui(
    *, data_utils: dict[str, Any], data_inputs: dict[str, Any]) -> Any:  # pragma: no cover
    """Create the goals subtab UI for primary and partner client inputs."""
    try:
        return ui.div(
            # Main section header
...
@module.server
def subtab_clients_goals_server(  # pragma: no cover
    input: typing.Any,
    output: typing.Any,
    session: typing.Any,
    ...
    """Run reactive goal formatting, totals, table rendering, and shared-state updates."""
    ...
    def observer_update_primary_essential_goal_formatting() -> None:
        ...
        try:
            ...
            constrained_value = validate_and_constrain_numeric_value(value = numeric_value, min_value = 0, max_value = 50000000)
    ...
    def output_ID_tab_clients_subtab_clients_goals_client_primary_total():
        """Calculate and display the total goals for the primary client."""
        try:
            ...
            value_Primary_Essential_Goal = get_value_from_shiny_input_numeric(
                input_reactive_object = input["input_ID_tab_clients_subtab_clients_goals_client_primary_goal_essential"],
            )
            value_Primary_Important_Goal = get_value_from_shiny_input_numeric(
                input_reactive_object = input["input_ID_tab_clients_subtab_clients_goals_client_primary_goal_important"],
            )
            value_Primary_Aspirational_Goal = get_value_from_shiny_input_numeric(
                input_reactive_object = input["input_ID_tab_clients_subtab_clients_goals_client_primary_goal_aspirational"],
            )
            ...
            return format_currency_value(amount = value_Primary_Total)
    ...
    def output_ID_tab_clients_subtab_clients_goals_client_partner_total():
        """Calculate and display the total goals for the partner client."""
        try:
            ...
            value_Partner_Essential_Goal = get_value_from_shiny_input_numeric(
                input_reactive_object = input["input_ID_tab_clients_subtab_clients_goals_client_partner_goal_essential"],
            )
            value_Partner_Important_Goal = get_value_from_shiny_input_numeric(
                input_reactive_object = input["input_ID_tab_clients_subtab_clients_goals_client_partner_goal_important"],
            )
            value_Partner_Aspirational_Goal = get_value_from_shiny_input_numeric(
                input_reactive_object = input["input_ID_tab_clients_subtab_clients_goals_client_partner_goal_aspirational"],
            )
            ...
            # Format for display (dollars only, no cents)
            return format_currency_value(amount = value_Partner_Total)
    ...
    def output_ID_tab_clients_subtab_clients_goals_combined_total():
        """Calculate and display the combined total goals for both clients."""
        try:
            ...
            # Format for display (dollars only, no cents)
            return format_currency_value(amount = value_Combined_Total)

        except Exception as exc_error:
            return f"Error: {exc_error!s}"

    @reactive.calc
    def calc_table_clients_goals_main() -> pl.DataFrame:
            ...
            try:
                ...
                if Goals_Table is None:
                    raise Exception_Validation_Input(
                        "Failed to create Goals_Table using enhanced function",
                    )
    ...
    def observer_update_shared_reactives_shiny_goals() -> None:
        """Mirror the current goal inputs into the shared reactive state."""
    ...
    def _poll_extracted_worksheet_for_goals():
    ...
    @reactive.poll(_poll_extracted_worksheet_for_goals, interval_secs=0.5)
    def _poll_goals_data():
        return _poll_extracted_worksheet_for_goals()

    @reactive.effect
    def observer_populate_goals_from_worksheet() -> None:  # pragma: no cover
        """Populate goals inputs from extracted worksheet PDF data.

        Uses :func:`map_goals_worksheet_to_inputs` for pure mapping logic;
        this observer only dispatches ``ui.update_text`` calls.
        """
        data = _poll_goals_data()
        _logger.warning("goals_populate: FIRED | data_is_dict=%s", isinstance(data, dict))
        if not isinstance(data, dict):
            return
        section = data.get("Goals", {})
        if not isinstance(section, dict):
            _logger.warning("goals_populate: Goals section missing")
            return

        _logger.warning("goals_populate: processing | section keys=%s", list(section.keys()))
        ...
        for client_role in ("client_primary", "client_partner"):
            mapped = map_goals_worksheet_to_inputs(
                section = section,
                client_role = client_role,
            )
            _logger.warning("goals_populate: %s mapped count=%s", client_role, len(mapped))
            for input_id, value in mapped.items():
                ui.update_text(input_id, value = value)

        _logger.warning(
            "Goals inputs populated from worksheet PDF",
            extra={"event_type": "worksheet_import", "subtab": "goals"},
        )
    ...
    # ------------------------------------------------------------------
    @reactive.effect
    def observer_deferred_populate_goals() -> None:  # pragma: no cover
        """Re-populate goals inputs when the subtab becomes visible."""
        inner_vars = reactives_shiny.get("Inner_Variables_Shiny", {})
        rv_data = inner_vars.get("Extracted_Worksheet_Data")
        if rv_data is None:
            return
        ...
        _logger.info(
            "Goals inputs deferred-populated from worksheet PDF",
            extra={"event_type": "worksheet_import", "subtab": "goals", "deferred": True},
        )

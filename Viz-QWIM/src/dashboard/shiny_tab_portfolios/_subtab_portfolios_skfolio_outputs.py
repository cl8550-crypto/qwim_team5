...
def compute_skfolio_performance_series_impl_QWIM(
    *, portfolio_1: Any, portfolio_2: Any, etf_data: pl.DataFrame | None, time_period: Any) -> tuple[pl.DataFrame | None, pl.DataFrame | None]:
    ...
    selected_period = str(time_period or "all")
...
def build_skfolio_statistics_rows_impl_QWIM(
    *, series_1: pl.DataFrame | None, series_2: pl.DataFrame | None, has_quantstats: bool, qs_module: Any) -> list[dict[str, Any]]:
    ...
    if has_quantstats:
        ...
        def _metric_cagr(*, returns_series: pd.Series) -> float:  # type: ignore[misc]
            returns_array = np.array(returns_series, dtype=np.float64)
            n_periods = len(returns_array)
            cumulative = float(np.prod(1.0 + returns_array))
            return float(cumulative ** (252.0 / n_periods) - 1.0) if n_periods > 0 else 0.0

        def _metric_volatility(*, returns_series: pd.Series) -> float:  # type: ignore[misc]
            returns_array = np.array(returns_series, dtype=np.float64)
            return float(np.std(returns_array, ddof=1) * np.sqrt(252.0)) if len(returns_array) > 1 else 0.0

        def _metric_sharpe(*, returns_series: pd.Series) -> float:  # type: ignore[misc]
            cagr_value, volatility_value = _metric_cagr(returns_series = returns_series), _metric_volatility(returns_series = returns_series)
            return float((cagr_value - 0.02) / volatility_value) if volatility_value > 1e-12 else 0.0

        def _metric_sortino(*, returns_series: pd.Series) -> float:  # type: ignore[misc]
            returns_array = np.array(returns_series, dtype=np.float64)
            cagr_value = _metric_cagr(returns_series = returns_series)
            downside = returns_array[returns_array < 0.0]
            downside_volatility = (
                float(np.std(downside, ddof=1) * np.sqrt(252.0)) if len(downside) > 1 else 1e-12
            )
            return float((cagr_value - 0.02) / downside_volatility) if downside_volatility > 1e-12 else 0.0

        def _metric_max_drawdown(*, returns_series: pd.Series) -> float:  # type: ignore[misc]
            returns_array = np.array(returns_series, dtype=np.float64)
            cumulative = np.cumprod(1.0 + returns_array)
            peak = np.maximum.accumulate(cumulative)
            return float(np.min(cumulative / peak - 1.0))

        def _metric_calmar(*, returns_series: pd.Series) -> float:  # type: ignore[misc]
            cagr_value = _metric_cagr(returns_series = returns_series)
            max_drawdown_value = abs(_metric_max_drawdown(returns_series = returns_series))
    ...
    for metric_name, metric_func, metric_format in metric_specs:
        metric_1 = _safe_metric(metric_func = metric_func, returns_series = returns_1)
        metric_2 = _safe_metric(metric_func = metric_func, returns_series = returns_2)
        rows.append(
            {
                "metric": metric_name,
                "method1": metric_1,
...
def render_skfolio_weights_plot_impl_QWIM(
    *, portfolio_1: Any, portfolio_2: Any, reactives_shiny: dict[str, Any]) -> Any:
    ...
    if portfolio_1 is None or portfolio_2 is None:
        return create_error_figure(
            ...
            title_text = "Click 'Run Optimization' button to compare portfolios",
        )
    ...
    try:
        ...
        plotnine_fig = build_plotnine_skfolio_weights_comparison(
            assets = assets,
        ...
        update_visual_object_in_reactives(
            ...
            chart_key = "Chart_skfolio_Optimization_Portfolio_Weights_Comparison",
            figure = plotnine_fig,
        )
        return fig
...
def render_skfolio_results_table_impl_QWIM(
    ...
    try:
        ...
        return ui.HTML(
            ...
            <div class=\"mt-3 p-3 bg-light rounded\">
                ...
                <p class=\"mb-0 text-muted small\"><i>Number of assets: {len(assets)} | Optimization date: {weights_1["Date"][0]}</i></p>
            </div>
            """,
        )
    except Exception as exc:
...
def render_skfolio_statistics_table_impl_QWIM(
    *, portfolio_1: Any, portfolio_2: Any, stats_rows: list[dict[str, Any]]) -> Any:
    ...
    table_rows = [
        (
            "<tr>"
            "<th>Metric</th>"
            f"<th>{portfolio_1.get_portfolio_name}</th>"
            f"<th>{portfolio_2.get_portfolio_name}</th>"
            "<th>Difference</th>"
            "</tr>"
        ),
    ]
    for row in stats_rows:
        ...
        table_rows.append(
            (
                ...
                f"<td>{method_1_string}</td>"
                f"<td>{method_2_string}</td>"
                f"<td class='{diff_color}'>{diff_string}</td>"
                "</tr>"
            ),
        )
...
def render_skfolio_performance_plot_impl_QWIM(
    *, portfolio_1: Any, portfolio_2: Any, perf_1: pl.DataFrame | None, perf_2: pl.DataFrame | None, reactives_shiny: dict[str, Any]) -> Any:
    ...
    try:
        ...
        plotnine_fig = build_plotnine_skfolio_performance_comparison(
            perf1_df = perf_1,
            perf2_df = perf_2,
            name1=portfolio_1.get_portfolio_name,
            name2=portfolio_2.get_portfolio_name,
        ...
        update_visual_object_in_reactives(
            reactives_shiny = reactives_shiny,
            chart_key = "Chart_skfolio_Optimization_Comparison_Portfolio_Performance",
            figure = plotnine_fig,
        )
        return fig
    except Exception as exc:

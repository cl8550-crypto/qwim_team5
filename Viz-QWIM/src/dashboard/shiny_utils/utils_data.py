...
def standardize_date_column(*, df: pl.DataFrame) -> pl.DataFrame:
    ...
    """
    date_col = find_date_column(df = df)
...
def get_input_data_raw(*, project_dir: Path) -> dict[str, pl.DataFrame]:
    ...
    try:
        ...
        if find_date_column(df = data_time_series_sample) is None:
            _logger.warning(
        ...
        data_time_series_sample = standardize_date_column(df = data_time_series_sample)
    ...
    except pl.exceptions.ComputeError as compute_exc:  # pragma: no cover  # reason: Polars ComputeError requires memory/binary corruption; not triggerable in unit tests
        raise Exception_Data_Not_Found(
    ...
    except pl.exceptions.SchemaError as schema_exc:  # pragma: no cover  # reason: Polars SchemaError requires malformed binary data; not triggerable in unit tests
        raise Exception_Data_Not_Found(
    ...
    try:
        ...
        data_time_series_etfs = standardize_date_column(df = data_time_series_etfs)
    ...
    try:
        ...
        # Standardize date column name
        data_weights_etfs = standardize_date_column(df = data_weights_etfs)
...
def get_input_data_processed(*, project_dir: Any) -> Any:
    ...
    try:
        ...
        # Check for required date column (case-insensitive)
        if find_date_column(df = data_portfolio_benchmark) is None:
            raise Exception_Validation_Input(
                f"Benchmark portfolio data file missing required date column: {path_file_portfolio_benchmark}",
            )

        # Standardize date column name
        data_portfolio_benchmark = standardize_date_column(df = data_portfolio_benchmark)
    ...
    try:
        ...
        if data_my_portfolio.is_empty():
            raise Exception_Validation_Input(
                f"Sample portfolio data file is empty: {path_file_my_portfolio}",
            )

        # Check for required date column (case-insensitive)
        if find_date_column(df = data_my_portfolio) is None:
            raise Exception_Validation_Input(
                f"My portfolio data file missing required date column: {path_file_my_portfolio}",
            )

        # Standardize date column name
        data_my_portfolio = standardize_date_column(df = data_my_portfolio)
...
def get_names_time_series_from_DF(*, polars_DF: Any) -> Any:
    ...
    try:
        ...
        raise Exception_Configuration(
            f"Error getting series names from DataFrame: {exc!s}",
        ) from exc
...
def downsample_dataframe(*, polars_DF: Any, max_points: Any = 200, date_column: Any = "Date") -> Any:
    ...
    Raises
    ------
        Exception_Validation_Input: If polars_DF is None, empty, or max_points is boolean
    ...
    if isinstance(max_points, bool):
        raise Exception_Validation_Input(
            "downsample_dataframe requires max_points to be an integer threshold, not a boolean.",
        )
    ...
    try:
        ...
        # Use a ceiling-based stride so the sampled frame does not exceed max_points.
        step_size = max(1, (num_rows_current + max_points - 1) // max_points)

        # Perform downsampling with proper indexing
        return polars_DF[::step_size]

    except (ValueError, Exception_Validation_Input) as validation_exc:
        # Re-raise validation errors with context preserved
        raise validation_exc
    except Exception as exc:  # pragma: no cover  # reason: Polars DataFrame operations on validated input should not raise unexpected exceptions
        # Handle unexpected errors with comprehensive context
        raise Exception_Configuration(f"Error in downsample_dataframe: {exc!s}") from exc
...
def get_safe_num_rows_for_DF(*, input_DF: Any) -> Any:
    ...
    try:
        ...
        return 0
    except Exception:  # pragma: no cover  # reason: defensive fallback; .height and .shape[0] on valid DataFrame objects do not raise
        return 0
...
def validate_portfolio_data(*, data_portfolio: Any) -> Any:
    ...
    except Exception as date_check_exc:  # pragma: no cover  # reason: select().to_series() on a validated Polars DataFrame does not raise
    ...
    except Exception as value_check_exc:  # pragma: no cover  # reason: select().to_series() on a validated Polars DataFrame does not raise
    ...
    try:
        ...
        if date_dtype in [pl.Utf8, pl.String]:
            try:
                ...
                for date_format in date_formats:
                    try:
                        ...
                        if null_count_after == original_null_count:
                            ...
                            break
                    except Exception as _fmt_exc:  # pragma: no cover  # reason: str.to_date with strict=False returns nulls rather than raising; exception path not reachable in practice
                ...
                if date_series_converted is None or successful_format is None:
                    ...
                    except Exception:  # pragma: no cover  # reason: str.to_datetime with strict=False returns nulls rather than raising
                        date_series_converted = None
                ...
                if date_series_converted is None or successful_format is None:
                    try:
                        ...
                        if (
                            (null_count_after - original_null_count) > (total_rows * 0.5)
                        ):  # pragma: no branch  # reason: the False branch requires exotic mixed-validity strings that pass only raw auto-parse; not practically reachable after all format fallbacks
                            return (
                                False,
                                f"Could not parse more than 50% of Date column strings. Please check date format. Sample values: {date_column.head(3).to_list()}",
                            )
                    except Exception as auto_parse_exc:  # pragma: no cover  # reason: str.to_date with strict=False returns nulls rather than raising
                        return (
                            False,
            ...
            except Exception as conversion_exc:  # pragma: no cover  # reason: outer defensive fallback; all inner exception handlers have already handled specific errors
                return (
                    False,
        ...
        if null_dates_count > original_null_count:
            return (  # pragma: no cover  # reason: all tested date formats fully convert their inputs; creating new nulls requires exotic mixed-validity date strings
                False,
    ...
    except Exception as date_order_exc:  # pragma: no cover  # reason: defensive fallback; validated Polars Series operations do not raise unexpected exceptions
    ...
    except Exception as value_null_exc:  # pragma: no cover  # reason: defensive fallback; null_count() on validated Polars Series does not raise
    ...
    except Exception as duplicate_check_exc:  # pragma: no cover  # reason: defensive fallback; n_unique() and len() on validated Polars Series do not raise
...
def calculate_portfolio_returns(*, data_portfolio: Any) -> Any:
    ...
    # Input validation with early returns
    if data_portfolio is None:
        raise Exception_Validation_Input("Portfolio data is None")
    ...
    except Exception as calc_exc:  # pragma: no cover  # reason: defensive fallback; Polars operations on pre-validated DataFrame do not raise unexpected exceptions
        raise Exception_Configuration(
            f"Error calculating portfolio returns: {calc_exc}",
        ) from calc_exc


def validate_portfolio_and_benchmark_data_if_not_already_validated(
    *, data_portfolio: pl.DataFrame | None, data_benchmark: pl.DataFrame | None) -> tuple[bool, bool]:

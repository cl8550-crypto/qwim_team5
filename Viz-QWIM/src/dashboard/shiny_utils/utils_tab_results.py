...
def process_data_for_plot_tab_results(
    *, data_frame: Any, date_column: Any = "date", max_points: Any = 5000) -> Any:
...
def transform_data_tab_results(
    *, data_frame: Any, transformation: Any = "none", baseline_date: Any = None) -> Any:
    ...
    if transformation == "percent_change":
        ...
        # Calculate relative change from baseline date — pure polars implementation
        from datetime import date as _date

        base_dt = _date.fromisoformat(str(baseline_date)[:10])

        # Find the closest date to baseline using polars
        data_frame = data_frame.with_columns(
            (pl.col("date").cast(pl.Date) - pl.lit(base_dt)).abs().alias("_date_diff"),
        )
        min_diff = data_frame["_date_diff"].min()
        baseline_row = data_frame.filter(pl.col("_date_diff") == min_diff)

        for col in series_columns:
            baseline_val = baseline_row[col][0] if baseline_row.height > 0 else None
            if baseline_val is not None and baseline_val != 0:
                data_frame = data_frame.with_columns(
                    ((pl.col(col) / baseline_val - 1) * 100).alias(f"{col}_rel"),
                )

        data_frame = data_frame.drop("_date_diff")

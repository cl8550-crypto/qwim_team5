"""Locale-aware formatting helpers for dashboard financial displays.

This module centralizes currency, percentage, date, and numeric formatting
with defensive fallbacks suitable for reactive Shiny UI usage.
"""
...
def validate_locale_setting(*, locale_setting: str) -> str:
    ...
    if not locale_setting.strip():
        raise Exception_Validation_Input("Empty locale setting provided. Using fallback locale.")
...
def validate_currency_code(*, currency_code: str) -> str:
    ...
    raise Exception_Validation_Input(
        f"Invalid currency code format '{currency_code}'. Using fallback: USD",
    )


def format_enhanced_currency_display(
    *, amount: int | float | str | Decimal, currency_code: str = "USD", locale_setting: str = "en_US", format_pattern: str = "standard", decimal_places: int | None = None) -> str:
    ...
    try:
        ...
        if (
            ...
            and not isinstance(decimal_places, bool)
...
def format_enhanced_percentage_display(
    *, value: int | float | str | Decimal, decimal_places: int = 2, locale_setting: str = "en_US", show_positive_sign: bool = False, multiply_by_hundred: bool = True) -> str:
    ...
    try:
        ...
        if isinstance(decimal_places, bool) or not isinstance(decimal_places, int) or not (0 <= decimal_places <= 10):
            decimal_places = 2
        ...
        if isinstance(value, str):
            ...
            try:
                ...
                raise Exception_Validation_Input(
                    f"Cannot convert '{value}' to a valid percentage",
                ) from exc_error
...
def format_enhanced_number_display(
    *, number: int | float | str | Decimal, decimal_places: int = 0, locale_setting: str = "en_US", show_thousands_separator: bool = True, show_positive_sign: bool = False, minimum_digits: int | None = None) -> str:
    ...
    try:
        ...
        # Validate decimal_places parameter
        if isinstance(decimal_places, bool) or not isinstance(decimal_places, int) or not (0 <= decimal_places <= 15):
            decimal_places = 0

        # Validate minimum_digits parameter
        if minimum_digits is not None and (
            isinstance(minimum_digits, bool)
        ...
        if isinstance(number, str):
            ...
            try:
                ...
                raise Exception_Validation_Input(
                    f"Cannot convert '{number}' to a valid number",
                ) from exc_error
        ...
        try:
            fallback_number = (
                float(number)
                if isinstance(number, (int, float, str)) and not isinstance(number, bool)
                else 0.0
            )
            if show_thousands_separator:
...
def format_enhanced_date_display(
    *, date_value: Any, format_style: str = "medium", locale_setting: str = "en_US", include_time: bool = False) -> str:
...
def configure_enhanced_formatting_module(
    *, custom_locales: list[str] | None = None, custom_fallback: str | None = None, log_level: str = "INFO") -> None:
    ...
    try:
        ...
        if custom_locales:
            if isinstance(custom_locales, list) and all(
                ...
                raise Exception_Validation_Input(
                    "Invalid custom_locales format. Must be a list of strings.",
                )

        # Update fallback locale if provided
        if custom_fallback:
            try:
                validate_locale_setting(locale_setting = custom_fallback)
                FALLBACK_LOCALE = custom_fallback
            except (TypeError, ValueError):
                raise Exception_Validation_Input(
                    f"Invalid custom fallback locale: {custom_fallback}",
                )

...
.. code-block:: python
    ...
    from src.utils.data_utils.generate_data import main
...
def _resolve_random_seed_QWIM(*, random_seed: int | None) -> int | None:
    """Return a safe RNG seed while keeping booleans on the default seed path."""
...
def _resolve_project_root(
    *, fallback_depth: int = 3) -> Path:
    """Resolve the repository root for generated data files.

    Prefer a directory that exposes repository markers so the function works
    both in the real repository layout and in tests that monkeypatch
    ``__file__`` into a temporary directory tree.
    """
    if isinstance(fallback_depth, bool):
        fallback_depth = 3
    ...
    fallback_index = max(0, min(fallback_depth - 1, len(resolved_file.parents) - 1))
    return resolved_file.parents[fallback_index]


def generate_monthly_timeseries(
    *, start_date: str = "2002-01-01", end_date: str = "2026-05-09") -> pl.DataFrame:
    ...
    end_date : str, optional
        End date in YYYY-MM-DD format, by default "2026-05-09".
...
def generate_scenarios_daily_returns_CMA_Tier_0(
    *, start_date: str = "2026-01-01", end_date: str = "2075-12-31", num_scenarios: int = 5000, random_seed: int | None = 42) -> dict[str, pl.DataFrame]:
    ...
    """
    random_seed = _resolve_random_seed_QWIM(random_seed = random_seed)
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        returns_outer = np.outer(daily_returns, daily_returns)
        ratio_matrix = covariance_daily / returns_outer
        argument_matrix = 1.0 + ratio_matrix

    sigma_ln = np.zeros((num_assets, num_assets), dtype=np.float64)
    valid_argument_mask = argument_matrix > 0.0
    sigma_ln[valid_argument_mask] = np.log(argument_matrix[valid_argument_mask])

    diagonal_indices = np.diag_indices(num_assets)
    diagonal_invalid_mask = argument_matrix[diagonal_indices] <= 0.0
    if np.any(diagonal_invalid_mask):
        diagonal_fallback = np.log(1.0 + (daily_vols / daily_returns) ** 2)
        sigma_ln[diagonal_indices] = np.where(
            diagonal_invalid_mask,
            diagonal_fallback,
            sigma_ln[diagonal_indices],
        )
    ...
    try:
        ...
        L = np.linalg.cholesky(sigma_ln)
        _logger.warning("Covariance matrix was not PSD; eigenvalues clipped to 1e-10")
    ...
    # --- Save to XLSX ---
    project_dir = _resolve_project_root(fallback_depth=4)
...
def main() -> None:
    ...
    --------
    >>> from src.utils.data_utils.generate_data import main
    >>> main()  # Logs progress through logger_custom
    """
    # Define project paths
    project_dir = _resolve_project_root(fallback_depth=3)
    data_dir = project_dir / "inputs"
    raw_data_dir = data_dir / "raw"
    output_file = raw_data_dir / "data_timeseries.csv"

    # Generate the data
    _logger.info("Generating time series data...")

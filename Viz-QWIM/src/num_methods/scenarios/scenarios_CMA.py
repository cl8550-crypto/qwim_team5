...
from ._scenario_rng import create_scenario_rng_QWIM
...
class Scenarios_CMA(Scenarios_Base):
    ...
    def __init__(
        ...
        """Initialize the CMA scenario set with asset-class return assumptions and a correlation matrix."""
    ...
    def get_asset_classes_by_tier(
        self, *, tier: Asset_Class_Tier) -> list[str]:
    ...
    def from_spreadsheet(
        cls, *, path: str | Path, sheet_name_returns: str = "Expected Returns", sheet_name_volatilities: str = "Volatilities", sheet_name_correlations: str = "Correlations", sheet_name_scenarios: str | None = None, dates: Sequence[dt.date] | None = None, start_date: dt.date | None = None, num_days: int = 252, num_scenarios: int = 1, data_type: Scenario_Data_Type = Scenario_Data_Type.RETURN_ARITHMETIC, frequency: Frequency_Time_Series = Frequency_Time_Series.DAILY, name_scenarios: str = "CMA Spreadsheet", index_map: dict[str, str] | None = None, tier_map: dict[str, Asset_Class_Tier] | None = None, random_seed: int | None = None) -> Scenarios_CMA:

r"""Abstract base class for daily financial scenario tables.

`Scenarios_Base` stores date-aligned multi-component scenario data in one
canonical `polars.DataFrame` and provides the shared validation surface for
...
class Scenarios_Base(ABC):
    ...
    def __init__(
        ...
        """Initialize the scenario set with component names, dates, and scenario metadata."""
        # --- validate components ---
        ...
        if dates is not None:
            self.m_dates: list[dt.date] = self._parse_dates(dates = dates)
    ...
    def filter_by_date_range(
        self, *, start_date: dt.date, end_date: dt.date) -> pl.DataFrame:
    ...
    def select_components(
        self, *, component_names: Sequence[str]) -> pl.DataFrame:
    ...
    def convert_prices_to_returns(
        self, *, log_returns: bool = False) -> pl.DataFrame:
    ...
    def convert_returns_to_prices(
        self, *, initial_prices: Sequence[float] | None = None) -> pl.DataFrame:
    ...
    @classmethod
    def from_csv(
        cls, *, path: str, data_type: Scenario_Data_Type = Scenario_Data_Type.RETURN_ARITHMETIC, frequency: Frequency_Time_Series = Frequency_Time_Series.DAILY, name_scenarios: str = "Loaded Scenario") -> Scenarios_Base:
        ...
        class _Loaded_Scenarios(Scenarios_Base):
            ...
            def generate(self) -> pl.DataFrame:
                """Return the pre-loaded scenarios DataFrame."""
    ...
    def _generate_business_dates(
        *, start_date: dt.date, num_days: int) -> list[dt.date]:

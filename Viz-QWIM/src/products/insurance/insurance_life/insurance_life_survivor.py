...
class Insurance_Life_Survivor(Insurance_Life_Base):
    ...
    **Joint COI** (simplified):
    ...
    def __init__(
        ...
        _logger.info(
            f"Created Survivor Life ({chassis.value}): face ${face_amount:,.2f},"
            f" insured ages {insured_age}/{insured_age_second}, split option: {has_split_option}"
        )
    ...
    def calc_premium_savings_vs_individual(
        self, *, individual_premium_insured_1: float, individual_premium_insured_2: float) -> float:
    ...
    def calc_estate_tax_coverage(
        self, *, estate_value: float, estate_tax_rate: float = 0.40, unified_credit_exclusion: float = 12_920_000.0) -> float:

...
class Insurance_Life_Base(ABC):
    ...
    def __init__(
        ...
        if isinstance(insured_age, bool) or not isinstance(insured_age, int) or insured_age < 0 or insured_age > 120:
        ...
        if isinstance(face_amount, bool) or not isinstance(face_amount, (int, float)) or face_amount <= 0:
            raise Exception_Validation_Input(
        ...
        _logger.info(
            ...
            f" face amount ${face_amount:,.2f}, class: {underwriting_class.value},"
            f" smoker: {is_smoker}"
        )

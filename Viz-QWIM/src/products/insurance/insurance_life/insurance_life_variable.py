...
class Insurance_Life_Variable(Insurance_Life_Base):
    ...
    **Monthly cash-value accumulation** (simplified):
    ...
    def __init__(
        ...
        _logger.info(
            "Created Variable Life: face ${:,.2f}, {} sub-account(s), CV ${:,.2f}, M&E {:.2%}",
            face_amount,
            len(sub_account_allocations),
            cash_value,
            rate_ME_charge,
        )
    ...
    def calc_weighted_return(
        self, *, sub_account_returns: dict[Sub_Account_Type, float]) -> float:
    ...
    def calc_cash_value_at_month(
        self, *, months: int, sub_account_monthly_return: float = 0.005) -> float:

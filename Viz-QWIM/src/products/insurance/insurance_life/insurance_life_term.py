...
class Insurance_Life_Term(Insurance_Life_Base):
    ...
    **Decreasing term face amount in year** $t$:
    ...
    def __init__(
        ...
        _logger.info(
            f"Created Term Life: face ${face_amount:,.2f}, term {term_years} yr ({term_type.value}),"
    ...
    def calc_total_premiums_paid(self) -> float:
        ...
        if self.m_term_type == Term_Type.ANNUALLY_RENEWABLE:
            total: float = 0.0
            for idx_year in range(1, self.m_term_years + 1):
                total += self.calc_annual_premium_at_year(policy_year = idx_year)
            return total
    ...
    def calc_cost_per_thousand_per_year(self, *, policy_year: int = 1) -> float:
        ...
        """
        premium = self.calc_annual_premium_at_year(policy_year = policy_year)
        db = self.calc_death_benefit_at_year(policy_year = policy_year)

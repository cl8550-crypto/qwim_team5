...
class Insurance_Life_Universal(Insurance_Life_Base):
    ...
    **Monthly cash-value accumulation**:
    ...
    def __init__(
        ...
        _logger.info(
            "Created Universal Life ({}): face ${:,.2f}, crediting rate {:.2%} "
            "(guaranteed {:.2%}), NLG: {}",
            ul_variant.value,
            face_amount,
            rate_crediting_current,
            rate_crediting_guaranteed,
            has_no_lapse_guarantee,
        )
    ...
    def calc_effective_crediting_rate(
        self, *, index_return: float = 0.0) -> float:
    ...
    def calc_cash_value_at_month(
        self, *, months: int, monthly_premium: float | None = None, index_return: float = 0.0) -> float:
        ...
        effective_rate = self.calc_effective_crediting_rate(index_return = index_return)
    ...
    def calc_months_until_lapse(
        self, *, monthly_premium: float = 0.0, index_return: float = 0.0) -> int:

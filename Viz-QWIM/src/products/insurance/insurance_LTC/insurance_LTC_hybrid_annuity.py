...
class Insurance_LTC_Hybrid_Annuity(Insurance_LTC_Base):
    ...
    **Monthly LTC benefit**:
    ...
    def __init__(
        ...
        _logger.info(
            "Hybrid Annuity LTC: deposit=${:,.2f}, multiplier={}x, annuity_type={}, "
            "crediting_rate={:.2%}",
            single_premium,
            LTC_multiplier.value,
            annuity_type.value,
            guaranteed_crediting_rate,
        )
    ...
    def calc_daily_benefit_at_year(self, *, year: int) -> float:
        ...
        av_at_year = self.calc_account_value_at_year(year = year)
    ...
    def calc_LTC_pool_at_year(self, *, year: int) -> float:
        ...
        Exception_Validation_Input
            If *year* is not a positive integer.
        """
        av = self.calc_account_value_at_year(year = year)
    ...
    def calc_monthly_LTC_benefit_at_year(self, *, year: int) -> float:
        ...
        Exception_Validation_Input
            If *year* is not a positive integer.
        """
        pool = self.calc_LTC_pool_at_year(year = year)
    ...
    def calc_surrender_value(self, *, year: int) -> float:
        ...
        av = self.calc_account_value_at_year(year = year)
        sc = self._get_surrender_charge_pct(year = year)
        net_av = av * (1.0 - sc)
    ...
    def calc_death_benefit_after_LTC(
        self, *, LTC_benefits_paid: float, year: int) -> float:
    ...
    def calc_remaining_LTC_pool(
        self, *, LTC_benefits_paid: float, year: int = 1) -> float:
    ...
    def calc_internal_rate_of_return_LTC(
        self, *, total_LTC_benefits_received: float, years_on_claim: int) -> float:
    ...
    def get_insurance_as_string(self) -> str:
        ...
        pool = self.calc_maximum_lifetime_benefit()
        monthly = self.calc_monthly_LTC_benefit_at_year(year = 1)

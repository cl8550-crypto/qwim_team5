...
class Insurance_LTC_Hybrid_Life(Insurance_LTC_Base):
    ...
    **Monthly LTC benefit**:
    ...
    def __init__(
        ...
        _logger.info(
            f"Hybrid Life LTC: DB=${death_benefit:,.2f}, premium=${single_premium:,.2f}"
            f" ({premium_structure.value}), extension={extension_of_benefits.value}x,"
    ...
    def calc_surrender_value(self, *, year: int) -> float:
        ...
        cv = self.calc_cash_value_at_year(year = year)
    ...
    def calc_residual_death_benefit(
        self, *, LTC_benefits_paid: float) -> float:
    ...
    def calc_breakeven_year_vs_traditional(
        self, *, traditional_annual_premium: float) -> int:

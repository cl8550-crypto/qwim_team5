...
class Insurance_LTC_Traditional(Insurance_LTC_Base):
    ...
    def __init__(
        ...
        _logger.info(
            f"Traditional LTC: gender={gender.value}, health_class={health_class.value},"
            f" non_forfeiture={non_forfeiture.value}, shared_care={shared_care}"
        )
    ...
    def _lookup_base_rate(self) -> float:
        ...
        for (low, high), rate in _age_band_rates_male.items():  # pragma: no branch
    ...
    def calc_remaining_benefit_pool(
        self, *, benefits_paid_to_date: float, policy_year: int = 1) -> float:
        ...
        daily_at_year = self.calc_daily_benefit_at_year(year = policy_year)
    ...
    def calc_non_forfeiture_benefit(
        self, *, years_premiums_paid: int) -> float:
    ...
    def calc_elimination_period_out_of_pocket(
        self, *, daily_care_cost: float) -> float:
    ...
    def calc_benefit_duration_at_cost(
        self, *, daily_care_cost: float, policy_year: int = 1) -> float:
        ...
        pool_remaining = self.calc_remaining_benefit_pool(benefits_paid_to_date = 0.0, policy_year = policy_year)

"""Variable annuity product model with market-linked sub-account exposure.

The class captures rider charges, benefit-base rollup and ratchet behavior,
and the payout assumptions used by the shared annuity framework.
"""
...
class Annuity_VA(Annuity_Base):
    ...
    def __init__(
        ...
        if isinstance(rate_ME_charge, bool) or not isinstance(rate_ME_charge, (int, float)) or rate_ME_charge < 0:
            raise Exception_Validation_Input(
        ...
        if isinstance(rate_admin_fee, bool) or not isinstance(rate_admin_fee, (int, float)) or rate_admin_fee < 0:
            raise Exception_Validation_Input(
        ...
        if isinstance(pct_GLWB, bool) or not isinstance(pct_GLWB, (int, float)) or pct_GLWB < 0 or pct_GLWB > 1:
            raise Exception_Validation_Input(
    ...
    def calc_account_value_after_charges(
        self, *, amount_principal: float, gross_returns: list[float]) -> list[float]:
    ...
    def calc_death_benefit(
        self, *, amount_principal: float, account_value: float) -> float:
    ...
    def calc_GLWB_payout(
        ...
        _logger.info("GLWB payout: %.2f (base: %.2f)", payout, benefit_base)
        return payout
    ...
    def calc_annuity_payout(
        ...
        if obj_scenarios is not None:
            ...
            _logger.info("VA benefit base after market scenario: %.2f", benefit_base)
        else:
            benefit_base = self.calc_benefit_base_with_rollup(amount_principal = amount_principal)
            _logger.info("VA benefit base from rollup: %.2f", benefit_base)

        annual_payout: float = benefit_base * self.m_annuity_payout_rate
        _logger.info("VA annual payout: %.2f", annual_payout)

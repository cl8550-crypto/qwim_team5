...
class Annuity_DIA(Annuity_Base):
    ...
    def __init__(
        ...
        if isinstance(rate_mortality_credit, bool) or not isinstance(rate_mortality_credit, (int, float)) or rate_mortality_credit < 0:
            raise Exception_Validation_Input(
    ...
    def calc_future_value(
        self, *, amount_principal: float, growth_rate: float) -> float:
    ...
    def calc_commutation_value(
        self, *, amount_principal: float, discount_rate: float, remaining_payment_years: int) -> float:
        ...
        for idx_year in range(1, remaining_payment_years + 1):
            payout_year = base_payout * ((1 + self.m_rate_COLA) ** (idx_year - 1))
            commutation += payout_year / ((1 + discount_rate) ** idx_year)
    ...
    def calc_mortality_credit_value(
        self, *, amount_principal: float) -> float:

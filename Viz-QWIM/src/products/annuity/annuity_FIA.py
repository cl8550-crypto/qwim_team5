"""Fixed indexed annuity product model with floor, cap, and participation terms.

The class captures indexed-crediting behavior, guaranteed benefit-base growth,
and surrender provisions used by the shared annuity framework.
"""
...
class Annuity_FIA(Annuity_Base):
    ...
    def __init__(
        ...
        if (
            isinstance(rate_rollup_benefit, bool)
            or not isinstance(rate_rollup_benefit, (int, float))
            or rate_rollup_benefit < 0
        ):
            raise Exception_Validation_Input(
        ...
        if isinstance(spread_rate, bool) or not isinstance(spread_rate, (int, float)) or spread_rate < 0:
            raise Exception_Validation_Input(
        ...
        if isinstance(floor_rate, bool) or not isinstance(floor_rate, (int, float)):
            raise Exception_Validation_Input(
        ...
        if isinstance(rate_minimum_guarantee, bool):
            raise Exception_Validation_Input(
                "rate_minimum_guarantee must be numeric",
                field_name="rate_minimum_guarantee",
                expected_type=float,
                actual_value=rate_minimum_guarantee,
            )

        if isinstance(pct_minimum_guarantee_base, bool):
            raise Exception_Validation_Input(
                "pct_minimum_guarantee_base must be numeric",
                field_name="pct_minimum_guarantee_base",
                expected_type=float,
                actual_value=pct_minimum_guarantee_base,
            )
    ...
    def calc_benefit_base_with_rollup(
        self, *, amount_principal: float, years_deferred: int | None = None) -> float:
    ...
    def calc_account_value(
        self, *, amount_principal: float, index_returns: list[float]) -> list[float]:
        ...
        for year_return in index_returns:
            credited = self.calc_credited_rate(index_return = year_return)
    ...
    def calc_minimum_guaranteed_value(
        self, *, amount_principal: float, years: int) -> float:
    ...
    def calc_surrender_value(
        self, *, account_value: float, year: int) -> float:
    ...
    def calc_annuity_payout(
        ...
        if obj_scenarios is not None:
            ...
            if self.m_client_age <= self.m_age_max_ratchet:
                rollup_base = self.calc_benefit_base_with_rollup(amount_principal = amount_principal)
            ...
            benefit_base = self.calc_benefit_base_with_rollup(amount_principal = amount_principal)
            _logger.info("FIA benefit base from rollup: %.2f", benefit_base)

        annual_payout: float = benefit_base * self.m_annuity_payout_rate
        _logger.info("FIA annual payout: %.2f", annual_payout)

        return annual_payout

    def calc_withdrawal_rates(
        self, *, amount_principal: float, desired_WR: float | None = None, obj_scenarios: pl.DataFrame | None = None, obj_inflation: pl.DataFrame | None = None) -> Withdrawal_Rates:

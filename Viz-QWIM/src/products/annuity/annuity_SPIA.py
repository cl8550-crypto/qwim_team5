...
class Annuity_SPIA(Annuity_Base):
    ...
    def __init__(
        ...
        if isinstance(guarantee_period_years, bool) or not isinstance(guarantee_period_years, int) or guarantee_period_years < 0:
            raise Exception_Validation_Input(
        ...
        if isinstance(rate_COLA, bool) or not isinstance(rate_COLA, (int, float)) or rate_COLA < 0:
            raise Exception_Validation_Input(
        ...
        if (
            isinstance(joint_survivor_pct, bool)
            or not isinstance(joint_survivor_pct, (int, float))
    ...
    def calc_payout_in_year(
        self, *, amount_principal: float, year_number: int) -> float:
        ...
        first_year_payout = self.calc_annuity_payout(amount_principal = amount_principal)
    ...
    def calc_present_value_payments(
        self, *, amount_principal: float, discount_rate: float, life_expectancy_years: int) -> float:
        ...
        for idx_year in range(1, num_years + 1):
            payout_year = first_year_payout * ((1 + self.m_rate_COLA) ** (idx_year - 1))
            present_value += payout_year / ((1 + discount_rate) ** idx_year)

        return present_value

    def calc_breakeven_age(
        self, *, amount_principal: float) -> int:
    ...
    def calc_exclusion_ratio(
        self, *, amount_principal: float, expected_payment_years: int) -> float:
        ...
        for idx_year in range(1, expected_payment_years + 1):
            total_expected += first_year_payout * ((1 + self.m_rate_COLA) ** (idx_year - 1))
    ...
    def calc_cumulative_payments(
        self, *, amount_principal: float, num_years: int) -> float:
        ...
        cumulative: float = 0.0
        for idx_year in range(1, num_years + 1):
            cumulative += first_year_payout * ((1 + self.m_rate_COLA) ** (idx_year - 1))

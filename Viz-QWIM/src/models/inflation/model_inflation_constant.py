...
class Inflation_Model_Constant(Inflation_Model_Base):  # noqa: N801
    ...
    def __init__(
        ...
        """Initialize the constant-inflation model with *annual_rate* and *name_model*."""
        super().__init__(name_model=name_model)

        if isinstance(annual_rate, bool) or not isinstance(annual_rate, float | int) or not np.isfinite(annual_rate):
    ...
    def fit(self, *, data: pl.DataFrame) -> Inflation_Model_Constant:
        ...
        if len(data) > 0:
            self._validate_inflation_data(data = data)
        ...
        # Commit state before any logging so a logging failure cannot leave
        # the model in an inconsistent NOT_FITTED state.
    ...
    def predict(
        ...
        dates = _make_date_range(start_date = start_date, n_periods = n_periods, freq = freq)

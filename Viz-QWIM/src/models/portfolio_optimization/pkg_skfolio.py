"""Adapters for portfolio-optimization workflows backed by `skfolio`.

This module wraps the external optimizer APIs used by QWIM and translates
their fitted weights into the project's portfolio objects.
"""
...
# pandas-boundary: skfolio requires pandas DataFrame input at this adapter seam.
import pandas as pd
import polars as pl
...
def _validate_returns_data(*, returns_data: pl.DataFrame) -> tuple[bool, str]:
    ...
    for col in asset_columns:
        ...
        if series_asset.dtype not in [pl.Float64, pl.Float32, pl.Int64, pl.Int32]:
            return False, f"Column '{col}' must be numeric, got {series_asset.dtype}"

        if series_asset.null_count() > 0:
            return False, f"Column '{col}' cannot contain null values"

        if not series_asset.cast(pl.Float64).is_finite().all():
            return False, f"Column '{col}' must contain only finite numeric values"
...
def _convert_polars_to_pandas_returns(*, returns_data: pl.DataFrame) -> pd.DataFrame:
    ...
    if "Date" in df_pandas.columns:
        df_pandas["Date"] = pd.to_datetime(df_pandas["Date"], utc=True)
        df_pandas = df_pandas.set_index("Date")

    return df_pandas


def _validate_benchmark_returns_tracking(
    *, returns_data: pl.DataFrame, benchmark_returns: pl.DataFrame | None) -> tuple[pl.DataFrame, pl.DataFrame]:
    """Validate and align benchmark returns for benchmark-tracking optimization.

    Parameters
    ----------
    returns_data : pl.DataFrame
        Portfolio returns with one ``Date`` column and one or more asset columns.
    benchmark_returns : pl.DataFrame | None
        Benchmark returns with one ``Date`` column and exactly one benchmark
        return column.

    Returns
    -------
    tuple[pl.DataFrame, pl.DataFrame]
        Returns-data and benchmark-data frames sorted by ``Date`` and verified
        to have matching ``Date`` values.

    Raises
    ------
    Exception_Validation_Input
        If the benchmark frame is missing, invalid, has more than one asset
        column, has a different row count, or does not share the same ``Date``
        values as ``returns_data``.
    """
    if benchmark_returns is None:
        raise Exception_Validation_Input(
            "benchmark_returns required for CONVEX_BENCHMARK_TRACKING",
        )

    is_valid_benchmark, error_msg_benchmark = _validate_returns_data(returns_data = benchmark_returns)
    if not is_valid_benchmark:
        raise Exception_Validation_Input(f"Invalid benchmark_returns: {error_msg_benchmark}")

    benchmark_columns = [col for col in benchmark_returns.columns if col != "Date"]
    if len(benchmark_columns) != 1:
        raise Exception_Validation_Input(
            "benchmark_returns must contain exactly one asset column for tracking",
        )

    if len(returns_data) != len(benchmark_returns):
        raise Exception_Validation_Input(
            "returns_data and benchmark_returns must have the same length",
        )

    returns_data_sorted = returns_data.sort("Date")
    benchmark_returns_sorted = benchmark_returns.sort("Date")
    if not returns_data_sorted.get_column("Date").equals(
        benchmark_returns_sorted.get_column("Date")
    ):
        raise Exception_Validation_Input(
            "returns_data and benchmark_returns must contain matching Date values",
        )

    return returns_data_sorted, benchmark_returns_sorted


def _extract_weights_to_portfolio_qwim(
    *, skfolio_model: Any, asset_names: list[str], portfolio_name: str, optimization_date: str | datetime | None = None) -> Portfolio_QWIM:
    ...
    Raises
    ------
        Exception_Validation_Input: If model weights and asset names do not align.
    ...
    if len(weights) != len(asset_names):
        raise Exception_Validation_Input(
            "skfolio_model.weights_ and asset_names must have the same length",
            field_name="skfolio_model.weights_ / asset_names",
            expected_type=list,
...
def _get_optimization_type_enum(
    *, optimization_type: str | portfolio_optimization_type) -> portfolio_optimization_type:
    ...
    Raises
    ------
        Exception_Validation_Input: If optimization_type is invalid.
    """
...
def calc_skfolio_optimization_basic(
    *, returns_data: pl.DataFrame, optimization_type: str
    | portfolio_optimization_type = portfolio_optimization_type.BASIC_EQUAL_WEIGHTED, portfolio_name: str | None = None, optimization_date: str | datetime | None = None, random_state: int | None = None, **kwargs: Any,
) -> Portfolio_QWIM:
    ...
    Raises
    ------
        Exception_Validation_Input: If returns_data is invalid or optimization_type is not a basic method.
    ...
    opt_type_enum = _get_optimization_type_enum(optimization_type = optimization_type)
    ...
    if opt_type_enum not in BASIC_OPTIMIZATION_MAPPING:
        ...
        _logger.error("Invalid optimization type: %s", error_msg)
        raise Exception_Validation_Input(error_msg)
    ...
    returns_pandas = _convert_polars_to_pandas_returns(returns_data = returns_data)
    asset_names = list(returns_pandas.columns)

    _logger.info(
        "Calculating basic optimization: type=%s, num_assets=%s",
        opt_type_enum.name,
        len(asset_names),
    )
    ...
    try:
        ...
        _logger.info(
            "Optimization complete: portfolio_name=%s, weights_sum=%s",
            portfolio_name,
            float(sum(optimizer.weights_)),
        )
        ...
        _logger.error("Basic optimization failed: %s", str(e))
...
def calc_skfolio_optimization_convex(
    *, returns_data: pl.DataFrame, optimization_type: str
    | portfolio_optimization_type = portfolio_optimization_type.CONVEX_MEAN_RISK, portfolio_name: str | None = None, optimization_date: str | datetime | None = None, objective_function: ObjectiveFunction = ObjectiveFunction.MINIMIZE_RISK, risk_measure: str | None = None, risk_aversion: float = 1.0, min_weights: float | None = 0.0, max_weights: float | None = 1.0, budget: float = 1.0, benchmark_returns: pl.DataFrame | None = None, **kwargs: Any,
) -> Portfolio_QWIM:
    ...
    Raises
    ------
        Exception_Validation_Input: If returns_data is invalid, optimization_type
        is not a convex method, or benchmark-tracking inputs are inconsistent.
    ...
    Notes
    -----
        This wrapper keeps the public validation contract in Polars, then
        translates the validated returns matrix into the pandas boundary that
        skfolio expects. The supported convex modes cover classical
        mean-variance optimization, equal-risk-contribution risk budgeting,
        maximum diversification, distributionally robust CVaR, and benchmark
        tracking. The implementation follows the same literature family cited
        by Markowitz (1952), Maillard, Roncalli, and Teiletche (2010), and
        Choueifaty and Coignard (2008), but keeps the runtime docstring in
        plain prose so the import-time metadata stays lightweight.
    """
    # Input validation with early returns
    is_valid, error_msg = _validate_returns_data(returns_data = returns_data)
    if not is_valid:
    ...
    benchmark_returns_validated: pl.DataFrame | None = None
    if opt_type_enum == portfolio_optimization_type.CONVEX_BENCHMARK_TRACKING:
        returns_data, benchmark_returns_validated = _validate_benchmark_returns_tracking(
    ...
    _logger.info(
        "Calculating convex optimization: type=%s, num_assets=%s, objective=%s",
        opt_type_enum.name,
        len(asset_names),
    ...
    if opt_type_enum == portfolio_optimization_type.CONVEX_MEAN_RISK:
        ...
        # Convert aligned benchmark to pandas.
        benchmark_pandas = _convert_polars_to_pandas_returns(returns_data = benchmark_returns_validated)
    ...
    try:
        ...
        _logger.error("Convex optimization failed: %s", str(e))
        raise Exception_Validation_Input(f"Optimization failed: {e!s}") from e
...
def calc_skfolio_optimization_clustering(
    *, returns_data: pl.DataFrame, optimization_type: str
    | portfolio_optimization_type = portfolio_optimization_type.CLUSTERING_HIERARCHICAL_RISK_PARITY, portfolio_name: str | None = None, optimization_date: str | datetime | None = None, risk_measure: str | None = None, hierarchical_clustering_estimator: Any | None = None, min_weights: float | None = 0.0, max_weights: float | None = 1.0, **kwargs: Any,
) -> Portfolio_QWIM:
    ...
    Raises
    ------
        Exception_Validation_Input: If returns_data is invalid or optimization_type is not a clustering method.
    ...
    _logger.info(
        "Calculating clustering optimization: type=%s, num_assets=%s",
        opt_type_enum.name,
        len(asset_names),
    )
    ...
    try:
        ...
        _logger.error("Clustering optimization failed: %s", str(e))
        raise Exception_Validation_Input(f"Optimization failed: {e!s}") from e
...
def calc_skfolio_optimization_ensemble(
    *, returns_data: pl.DataFrame, optimization_type: str
    | portfolio_optimization_type = portfolio_optimization_type.ENSEMBLE_STACKING, portfolio_name: str | None = None, optimization_date: str | datetime | None = None, estimators: list[Any] | None = None, final_estimator: Any | None = None, cv: int | None = None, **kwargs: Any,
) -> Portfolio_QWIM:
    ...
    Raises
    ------
        Exception_Validation_Input: If returns_data is invalid or optimization_type is not an ensemble method.
    ...
    _logger.info(
        "Calculating ensemble optimization: type=%s, num_assets=%s",
        opt_type_enum.name,
        len(asset_names),
    )
    ...
    try:
        ...
        _logger.error("Ensemble optimization failed: %s", str(e))
        raise Exception_Validation_Input(f"Optimization failed: {e!s}") from e

"""Adapters for portfolio-optimization workflows backed by `azapy`.

This module converts QWIM return tables to the pandas-based azapy API and
translates the resulting optimized weights into project portfolio objects.
"""
...
import azapy
# pandas-boundary: azapy requires pandas DataFrame input at this adapter seam.
...
VALID_RTYPES_AZAPY_MULTI_OBJECTIVE: tuple[str, ...] = (
    "MinRisk",
    "Sharpe",
    "Sharpe2",
    "InvNrisk",
...
VALID_RTYPES_AZAPY_KELLY: tuple[str, ...] = ("ExpCone", "Full", "Order2")
...
def _validate_rtype_azapy(
    *, rtype: object, valid_rtypes: tuple[str, ...]) -> str:
    """Validate azapy optimization objective labels before library calls.

    Parameters
    ----------
    rtype : object
        Candidate optimization objective label.
    valid_rtypes : tuple[str, ...]
        Supported azapy objective labels for the current analyzer.

    Returns
    -------
    str
        The validated objective label.

    Raises
    ------
    Exception_Validation_Input
        If ``rtype`` is not a string or is not supported by the analyzer.
    """
    if not isinstance(rtype, str):
        msg = (
            "rtype must be a string with one of "
            f"{valid_rtypes}, got {type(rtype).__name__}"
        )
        _logger.error(msg)
...
def _extract_weights_from_azapy(
    *, weights_series: pd.Series, portfolio_name: str, optimization_date: str | datetime | None = None) -> Portfolio_QWIM:
    ...
    ------
    Exception_Validation_Input
        If ``weights_series`` is empty.
    """
    if weights_series.empty:
        raise Exception_Validation_Input(
            "weights_series cannot be empty",
            field_name="weights_series",
            expected_type=pd.Series,
            actual_value="empty series",
        )
...
def calc_azapy_mean_variance(
    *, returns_data: pl.DataFrame, rtype: str = "MinRisk", aversion: float | None = None, portfolio_name: str | None = None, optimization_date: str | datetime | None = None) -> Portfolio_QWIM:
    ...
    rtype : str
        ...
        - ``"Sharpe2"`` : Alternative Sharpe solver used as a stable fallback
        - ``"InvNrisk"`` : Inverse normalized risk allocation
        - ``"RiskAverse"`` : Risk-aversion weighted portfolio
        - ``"Diverse"`` : Diversification-oriented allocation
        - ``"MaxDiverse"`` : Maximum diversification allocation
    ...
    rtype = _validate_rtype_azapy(rtype = rtype, valid_rtypes = VALID_RTYPES_AZAPY_MULTI_OBJECTIVE)
    ...
    try:
        ...
        if weights_series is None:
            raise Exception_Validation_Input("MVAnalyzer returned None weights")
        ...
        _logger.error("Mean-variance optimization failed: %s", str(e))
...
def calc_azapy_cvar(
    *, returns_data: pl.DataFrame, alpha: float = 0.975, rtype: str = "Sharpe", portfolio_name: str | None = None, optimization_date: str | datetime | None = None) -> Portfolio_QWIM:
    ...
    rtype : str
        ...
        ``"Sharpe2"``, ``"InvNrisk"``, ``"RiskAverse"``, ``"Diverse"``,
        ``"MaxDiverse"``. Default is ``"Sharpe"``.
    portfolio_name : str | None
        Name for the resulting portfolio. Defaults to a name based on rtype.
    optimization_date : str | datetime | None
        Date for the portfolio weights. Defaults to current UTC date.
    ...
    ------
    Exception_Validation_Input
        If ``returns_data`` is invalid, ``alpha`` is out of range, or ``rtype`` is invalid.
    ...
    try:
        ...
        if weights_series is None:
            raise Exception_Validation_Input("CVaRAnalyzer returned None weights")
        ...
        _logger.error("CVaR optimization failed: %s", str(e))
        raise Exception_Validation_Input(f"Optimization failed: {e!s}") from e
...
def calc_azapy_mad(
    *, returns_data: pl.DataFrame, rtype: str = "MinRisk", portfolio_name: str | None = None, optimization_date: str | datetime | None = None) -> Portfolio_QWIM:
    ...
    ------
    Exception_Validation_Input
        If ``returns_data`` is invalid or ``rtype`` is invalid.
    ...
    try:
        ...
        if weights_series is None:
            raise Exception_Validation_Input("MADAnalyzer returned None weights")
        ...
        _logger.error("MAD optimization failed: %s", str(e))
        raise Exception_Validation_Input(f"Optimization failed: {e!s}") from e
...
def calc_azapy_inverse_volatility(
    *, returns_data: pl.DataFrame, portfolio_name: str | None = None, optimization_date: str | datetime | None = None) -> Portfolio_QWIM:
    ...
    try:
        ...
        _logger.error("Inverse volatility portfolio failed: %s", str(e))
        raise Exception_Validation_Input(f"Optimization failed: {e!s}") from e
...
def calc_azapy_kelly(
    *, returns_data: pl.DataFrame, rtype: str = "ExpCone", portfolio_name: str | None = None, optimization_date: str | datetime | None = None) -> Portfolio_QWIM:
    ...
    rtype = _validate_rtype_azapy(rtype = rtype, valid_rtypes = VALID_RTYPES_AZAPY_KELLY)
    ...
    try:
        ...
        _logger.error("Kelly optimization failed: %s", str(e))
        raise Exception_Validation_Input(f"Optimization failed: {e!s}") from e
...
def calc_azapy_evar(
    *, returns_data: pl.DataFrame, alpha: float = 0.975, rtype: str = "Sharpe", portfolio_name: str | None = None, optimization_date: str | datetime | None = None) -> Portfolio_QWIM:
    ...
    try:
        ...
        if weights_series is None:
            raise Exception_Validation_Input("EVaRAnalyzer returned None weights")
        ...
        _logger.error("EVaR optimization failed: %s", str(e))
        raise Exception_Validation_Input(f"Optimization failed: {e!s}") from e

...
class Discounting_Model_Base(ABC):
    ...
    name_model: str = attrs.field()

    def __attrs_post_init__(self) -> None:
        """Validate the model name after attrs initialization.
        ...
        """
        if not isinstance(self.name_model, str) or len(self.name_model.strip()) == 0:
    ...
    def _calc_year_fraction(
        ...
        """
        resolved_start = self._resolve_start_date(start_date = start_date)
        ...
        calculator = get_daycount_calculator(convention = daycount_convention)
        return calculator.calc_year_fraction(date_start = resolved_start, date_end = end_date)

    def calc_present_value(
        self, *, cash_flow: float, end_date: date, start_date: date | None = None, daycount_convention: Daycount_Convention = Daycount_Convention.ACTUAL_ACTUAL) -> float:
    ...
    def calc_present_value_stream(
        self, *, cash_flows: list[float], end_dates: list[date], start_date: date | None = None, daycount_convention: Daycount_Convention = Daycount_Convention.ACTUAL_ACTUAL) -> float:
        ...
        return sum(
            self.calc_present_value(cash_flow = cf, end_date = ed, start_date = start_date, daycount_convention = daycount_convention)
    ...
    def __repr__(self) -> str:
        ...
        return f"{self.__class__.__name__}(name_model='{self.name_model}')"

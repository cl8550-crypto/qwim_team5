...
def _get_simulation_status(*, name_status: str) -> Simulation_Status:
    """Return a typed simulation-status member from the dynamic ``aenum`` class."""
    return cast(Simulation_Status, getattr(Simulation_Status, name_status))


SIMULATION_STATUS_NOT_STARTED = _get_simulation_status(name_status = "NOT_STARTED")
SIMULATION_STATUS_RUNNING = _get_simulation_status(name_status = "RUNNING")
SIMULATION_STATUS_COMPLETED = _get_simulation_status(name_status = "COMPLETED")
SIMULATION_STATUS_FAILED = _get_simulation_status(name_status = "FAILED")
...
class Simulation_Base(ABC):  # noqa: N801
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize the simulation with a scenario set, component names, and portfolio weights."""
        # --- validate scenarios ---
        ...
        if not np.all(np.isfinite(weights_arr)):
            raise Exception_Validation_Input(
                ...
                field_name="weights",
                expected_type=np.ndarray,
                actual_value=weights_arr,
            )

        weight_sum = float(np.sum(weights_arr))
        if np.isclose(weight_sum, 0.0, atol=1e-12):
            raise Exception_Validation_Input(
        ...
        if (
            ...
            or not isinstance(initial_value, float | int)
            or not np.isfinite(initial_value)
            or initial_value <= 0
        ):
            raise Exception_Validation_Input(
                "initial_value must be a positive finite number (> 0)",
        ...
        self.m_status: Simulation_Status = SIMULATION_STATUS_NOT_STARTED

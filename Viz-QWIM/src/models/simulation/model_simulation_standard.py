...
DEFAULT_DISTRIBUTION_TYPE_SIMULATION = cast(
    Distribution_Type,
    getattr(Distribution_Type, "NORMAL"),
)
"""Typed default distribution enum used in public simulation signatures."""
...
class Simulation_Standard(Simulation_Base):  # noqa: N801
    ...
    def __init__(
        ...
        *,
        names_components: Sequence[str],
        weights: np.ndarray,
        distribution_type: Distribution_Type = DEFAULT_DISTRIBUTION_TYPE_SIMULATION,
        ...
        """Initialize the Monte Carlo simulation with component names, weights, and distribution parameters."""
        # --- validate components ---
    ...
    def run(self) -> pl.DataFrame:
        ...
        ------
        Exception_Calculation
            If batch simulation fails before the results DataFrame is assembled.
        """
        self.m_status = SIMULATION_STATUS_RUNNING
        self.m_df_results = None
        _logger.info(
        ...
        try:
            ...
            self.m_status = SIMULATION_STATUS_FAILED
            _logger.exception(
        ...
        # Build scenario columns directly from numpy array (avoids Python loop + .tolist())
        scenario_cols = [f"Scenario_{idx_s + 1}" for idx_s in range(self.m_num_scenarios)]
        # portfolio_paths shape (T, N) — use orient="row" (default: rows=first dim, cols=second dim)
        df_scenarios = pl.from_numpy(
            data=portfolio_paths,
            schema=scenario_cols,
        )
        self.m_df_results = df_scenarios.with_columns(
            pl.Series("Date", dates, dtype=pl.Date),
        ).select(["Date"] + scenario_cols)

        self.m_status = SIMULATION_STATUS_COMPLETED
        _logger.info(
    ...
    def _run_batch(self) -> np.ndarray:
        ...
        mat_chol = self._safe_cholesky(matrix = self.m_covariance_matrix)
        ...
        if self.m_distribution_type == Distribution_Type.NORMAL:
            ...
            # Map to underlying normal parameters (vectorized)
            ratio = cov / (mu[:, None] * mu[None, :])
            arg = 1.0 + ratio
            if np.any(arg <= 0):
                bad_idx = np.argwhere(arg <= 0)[0]
                raise Exception_Calculation(
                    f"Lognormal mapping failed for ({int(bad_idx[0])}, {int(bad_idx[1])})",
                )
            sigma_ln = np.log(arg)

            mu_ln = np.log(mu) - 0.5 * np.diag(sigma_ln)
            mat_chol_ln = self._safe_cholesky(matrix = sigma_ln)
            ...
            mat_chol_t = self._safe_cholesky(matrix = scale)
    ...
    def from_historical_data(
        cls, *, price_data: pl.DataFrame, weights: np.ndarray, distribution_type: Distribution_Type = DEFAULT_DISTRIBUTION_TYPE_SIMULATION, initial_value: float = DEFAULT_INITIAL_VALUE, num_scenarios: int = DEFAULT_NUM_SCENARIOS, num_days: int = DEFAULT_NUM_DAYS, start_date: dt.date | None = None, random_seed: int | None = DEFAULT_RANDOM_SEED, degrees_of_freedom: float = 5.0, name_simulation: str = "Historical MC") -> Simulation_Standard:

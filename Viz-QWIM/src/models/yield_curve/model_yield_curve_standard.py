...
from src.models.yield_curve.model_yield_curve_base import (
    ...
    YIELD_CURVE_MODEL_STATUS_FITTED,
)
...
def _ns_yield(*, tau: float, beta0: float, beta1: float, beta2: float, lam: float) -> float:
    ...
    """
    _, l2, l3 = _ns_loadings(tau = tau, lam = lam)
...
def _design_matrix(*, taus: np.ndarray, lam: float) -> np.ndarray:
    ...
    np.ndarray
        ...
        ``[1, L2(tau), L3(tau)]``.
    """
    rows = [_ns_loadings(tau = float(t), lam = lam) for t in taus]
...
class Yield_Curve_Model_Standard(Yield_Curve_Model_Base):  # noqa: N801
    ...
    * The fitter requires at least **3** data points.
    ...
    def __init__(
        ...
        """Initialize the Nelson-Siegel yield-curve model with its shape parameters."""
        super().__init__(name_model=name_model)

        # lambda must be strictly positive (appears in denominator)
        _validate_positive_float(value = lambda_, name = "lambda_")

        # betas can be any finite real number
        _validate_finite_float(value = beta0, name = "beta0")
        _validate_finite_float(value = beta1, name = "beta1")
        _validate_finite_float(value = beta2, name = "beta2")
    ...
    def fit(self, *, data: pl.DataFrame) -> Yield_Curve_Model_Standard:
        ...
        if len(data) == 0:
            ...
            self.m_status = YIELD_CURVE_MODEL_STATUS_FITTED
            self.m_parameters = {
                "beta0": self.m_beta0,
                "beta1": self.m_beta1,
                "beta2": self.m_beta2,
        ...
        self._validate_yield_curve_data(data = data)
        ...
        for lam in _LAMBDA_GRID:
            xmat = _design_matrix(taus = taus, lam = float(lam))
            try:
    ...
    def predict(
        ...
        max_maturity : float, keyword-only, optional
            ...
            (default ``30.0``). Must be at least ``0.25``.
        ...
        yield_col: list[float] = [_ns_yield(tau = float(t), beta0 = beta0, beta1 = beta1, beta2 = beta2, lam = lam) for t in taus]
        fwd_col: list[float] = [_ns_forward(tau = float(t), beta0 = beta0, beta1 = beta1, beta2 = beta2, lam = lam) for t in taus]

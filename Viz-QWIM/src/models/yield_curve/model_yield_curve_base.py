...
MIN_YIELD_CURVE_GRID_MATURITY: float = 0.25
"""Lower bound in years for generated logarithmic maturity grids."""
...
def _get_yield_curve_model_status(*, name_status: str) -> Yield_Curve_Model_Status:
    """Return a typed status enum member from the dynamic ``aenum`` class."""
    return cast(Yield_Curve_Model_Status, getattr(Yield_Curve_Model_Status, name_status))


YIELD_CURVE_MODEL_STATUS_NOT_FITTED = _get_yield_curve_model_status(name_status = "NOT_FITTED")
YIELD_CURVE_MODEL_STATUS_FITTED = _get_yield_curve_model_status(name_status = "FITTED")
YIELD_CURVE_MODEL_STATUS_FAILED = _get_yield_curve_model_status(name_status = "FAILED")
...
class Yield_Curve_Model_Base(ABC):  # noqa: N801
    ...
    def __init__(self, name_model: str = "Yield Curve Model") -> None:
        """Initialize the yield-curve model with *name_model*."""
        ...
        self.name_model: str = name_model.strip()
        self.m_status: Yield_Curve_Model_Status = YIELD_CURVE_MODEL_STATUS_NOT_FITTED
    ...
    @abstractmethod
    def predict(
        self, *, maturities: list[float] | np.ndarray | None = None, n_points: int = 50, max_maturity: float = 30.0) -> pl.DataFrame:
    ...
    def is_fitted(self) -> bool:
        ...
        return self.m_status == YIELD_CURVE_MODEL_STATUS_FITTED
    ...
    def _resolve_maturities(
        self, *, maturities: list[float] | np.ndarray | None, n_points: int, max_maturity: float) -> np.ndarray:
        ...
        max_maturity : float
            ...
            is ``None``). Must be at least ``0.25`` so the generated
            logarithmic grid remains increasing.
        ...
        if (
            ...
            or float(max_maturity) < MIN_YIELD_CURVE_GRID_MATURITY
        ):
            raise Exception_Validation_Input(
                "max_maturity must be a finite float >= 0.25",
        ...
        from_log = np.log10(MIN_YIELD_CURVE_GRID_MATURITY)
    ...
    def __repr__(self) -> str:
        ...
        return f"Yield_Curve_Model_Base(name='{self.name_model}', status={self.m_status.value})"

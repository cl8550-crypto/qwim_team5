...
* **Auditability** — every state transition is written to the project
  _logger in ``%s`` format (no f-strings in log calls).
...
def _get_rebalancing_strategy_status(
    *, name_status: str) -> Rebalancing_Strategy_Status:
    """Return a typed rebalancing-status member from the dynamic ``aenum`` class."""
    return cast(
        Rebalancing_Strategy_Status,
        getattr(Rebalancing_Strategy_Status, name_status),
    )


REBALANCING_STRATEGY_STATUS_NOT_CONFIGURED = _get_rebalancing_strategy_status(
    name_status = "NOT_CONFIGURED",
)
REBALANCING_STRATEGY_STATUS_CONFIGURED = _get_rebalancing_strategy_status(
    name_status = "CONFIGURED",
)
REBALANCING_STRATEGY_STATUS_FAILED = _get_rebalancing_strategy_status(
    name_status = "FAILED",
)
...
class Portfolio_Rebalancing_Base(ABC):  # noqa: N801
    ...
    def __init__(self, name_strategy: str = "Portfolio Rebalancing") -> None:
        """Initialize the rebalancing strategy with *name_strategy*."""
        ...
        self.m_status: Rebalancing_Strategy_Status = (
            REBALANCING_STRATEGY_STATUS_NOT_CONFIGURED
        )
    ...
    @abstractmethod
    def configure(
        self, *, target_weights: np.ndarray, names_assets: list[str]) -> Portfolio_Rebalancing_Base:
    ...
    def should_rebalance(
        self, *, current_weights: np.ndarray, current_date: datetime | None = None) -> bool:
    ...
    @abstractmethod
    def rebalance(
        self, *, current_weights: np.ndarray, current_date: datetime | None = None, portfolio_value: float = 1_000_000.0) -> pl.DataFrame:
    ...
    def is_configured(self) -> bool:
        ...
        return self.m_status == REBALANCING_STRATEGY_STATUS_CONFIGURED
    ...
    def _validate_weights_array(
        self, *, weights: np.ndarray, field_name: str = "weights", must_sum_to_one: bool = True) -> np.ndarray:
        ...
        numeric values, rejects boolean entries, and, when
        ``must_sum_to_one`` is ``True``, checks that the values sum to
        1.0 within :data:`_WEIGHT_SUM_TOLERANCE`.
        ...
        raw_weights = np.asarray(weights, dtype=object)
        if any(isinstance(value, (bool, np.bool_)) for value in raw_weights.flat):
            raise Exception_Validation_Input(
                f"{field_name} must not contain bool values",
                field_name=field_name,
                expected_type=float,
                actual_value="bool entry",
            )
    ...
    def _validate_portfolio_value(
        self, *, portfolio_value: float, field_name: str = "portfolio_value") -> float:
    ...
    def _validate_asset_names(
        self, *, names_assets: list[str], n_assets_expected: int | None = None) -> list[str]:

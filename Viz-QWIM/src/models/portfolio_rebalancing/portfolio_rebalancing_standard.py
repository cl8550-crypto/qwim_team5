    ...
    * **Calendar trigger** (when
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize the threshold-band rebalancing strategy with tolerance and cost parameters."""
        super().__init__(name_strategy=name_strategy)

        self.m_tolerance_abs: float = self._init_tolerance_abs(value = tolerance_abs)
        self.m_frequency_days: int = self._init_frequency_days(value = rebalancing_frequency_days)
        self.m_cost_bps: float = self._init_cost_bps(value = transaction_cost_bps)
    ...
    def configure(
        self, *, target_weights: np.ndarray, names_assets: list[str]) -> Portfolio_Rebalancing_Standard:
        ...
        """
        names = self._validate_asset_names(names_assets = names_assets)
    ...
    def should_rebalance(
        ...
        if (
            ...
            _logger.debug("Calendar trigger fired: first evaluation with no prior rebalance")
            return True
    ...
    def get_turnover(self, *, current_weights: np.ndarray) -> float:
        ...
        return self._compute_turnover(trade_weights = trade_w)

    def get_estimated_cost(
        self, *, current_weights: np.ndarray, portfolio_value: float = 1_000_000.0) -> float:
        ...
        self._check_is_configured()
        pv = self._validate_portfolio_value(portfolio_value = portfolio_value)
        turnover = self.get_turnover(current_weights = current_weights)

...
def _get_longevity_model_status(
    *, name_status: str) -> Longevity_Model_Status:
    """Return a typed longevity-model status member from the dynamic ``aenum`` class."""
    return cast(Longevity_Model_Status, getattr(Longevity_Model_Status, name_status))


LONGEVITY_MODEL_STATUS_NOT_FITTED = _get_longevity_model_status(name_status = "NOT_FITTED")
LONGEVITY_MODEL_STATUS_FITTED = _get_longevity_model_status(name_status = "FITTED")
LONGEVITY_MODEL_STATUS_FAILED = _get_longevity_model_status(name_status = "FAILED")
...
class Longevity_Model_Base(ABC):  # noqa: N801
    ...
    def __init__(self, name_model: str = "Longevity Model") -> None:
        """Initialize the longevity model with *name_model*."""
        ...
        self.m_status: Longevity_Model_Status = LONGEVITY_MODEL_STATUS_NOT_FITTED
    ...
    def is_fitted(self) -> bool:
        ...
        return self.m_status == LONGEVITY_MODEL_STATUS_FITTED
    ...
    def _validate_n_ages(self, *, n_ages: int) -> None:
        ...
        ------
        Exception_Validation_Input
            If ``n_ages`` is boolean, is not an ``int``, or is less
            than 1.
        """
        if isinstance(n_ages, bool) or not isinstance(n_ages, int) or n_ages < 1:
            raise Exception_Validation_Input(
    ...
    def _validate_age(self, *, age: int) -> None:
        ...
        ------
        Exception_Validation_Input
            If ``age`` is boolean, is not an ``int``, or is negative.
        """
    ...
    def __repr__(self) -> str:
        ...
        return f"Longevity_Model_Base(name='{self.name_model}', status={self.m_status.value})"

...
class Longevity_Model_Standard(Longevity_Model_Base):  # noqa: N801
    ...
    def __init__(
        ...
        """Initialize the Gompertz mortality model with scale *B* and shape *b*."""
        super().__init__(name_model=name_model)

        _validate_positive_float(value = B, name = "B")
        _validate_positive_float(value = b, name = "b")
    ...
    def fit(self, *, data: pl.DataFrame) -> Longevity_Model_Standard:
        ...
        * :math:`\hat{B} = \exp(\hat\alpha) \cdot \hat{b} / (e^{\hat{b}} - 1)`
        ...
        Exception_Validation_Input
            ...
            filtering.
        """
        self._validate_mortality_data(data = data)
    ...
    def survival_probability(self, *, current_age: int, t_years: float) -> float:
        ...
        if (
            isinstance(t_years, bool)
            or not isinstance(t_years, float | int)
    ...
    def get_force_of_mortality(self, *, age: int | float) -> float:
        ...
        Exception_Calculation
            If the model has not been fitted yet.
        Exception_Validation_Input
            If *age* is not a finite non-negative real number.
        """
        self._check_is_fitted()
        if isinstance(age, bool) or not isinstance(age, float | int) or not np.isfinite(age) or age < 0:
            raise Exception_Validation_Input(
...
def _validate_positive_float(*, value: float, name: str) -> None:
    """Raise if *value* is not a positive finite real number.
